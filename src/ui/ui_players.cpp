...
tryagain:
  ...
  // NERVE - SMF - multiplayer only hack to show correct panzerfaust and
  // venom barrel
...
void UI_DrawPlayer(float x, float y, float w, float h, playerInfo_t *pi,
  ...
  //	UI_PlayerAnimation( pi, &legs.oldframe, &legs.frame,
  //&legs.backlerp, 		 &torso.oldframe, &torso.frame, &torso.backlerp
  //);
...
static qboolean UI_RegisterClientSkin(playerInfo_t *pi, const char *modelName,
  ...
  Com_sprintf(filename, sizeof(filename), "models/players/%s/body_%s.skin",
              modelName,
              skinName); // NERVE - SMF - make this work with wolf
...
static qboolean AnimParseAnimConfig(playerInfo_t *animModelInfo,
  ...
  for (i = 0; (animModelInfo->version > 1) || (i < MAX_ANIMATIONS); i++) {
    ...
    if (!animModelInfo->isSkeletal) // skeletal models dont
                                    // require adjustment
    { // leg only frames are adjusted to not count the upper
      // body only frames
      if (i == LEGS_WALKCR) {
  ...
  if (token && token[0]) {
    if (animModelInfo->version < 2 || !Q_stricmp(token, "HEADFRAMES")) {
      ...
      for (i = 0; i < MAX_HEAD_ANIMS; i++) {
        ...
        if (animModelInfo->version > 1) // includes animation names at
                                        // start of each line
        {                               // just throw this information away,
                                        // not required for head
          token = COM_ParseExt(&text_p, qfalse);
          if (!token || !token[0]) {
            break;
          }
...
qboolean UI_RegisterClientModelname(playerInfo_t *pi,
  ...
  // NERVE - SMF - determine skin
  {
    ...
    // DHM - Nerve :: Don't rely on cvar for team, use modelname
    // instead teamval = trap_Cvar_VariableValue( "mp_team" );
    ...
    if (var == 0) {
      ...
      if (teamval == 1) {
        ...
        backpack = "acc/backpack/"
                   "backpack_german_cvops.md3";
        helmet = "acc/helmet_german/"
                 "helmet_cvops.md3";
      }
    } else {
  ...
  // load the animations
  //----(SA) changing name of config file to avoid backwards or
  // alternate
