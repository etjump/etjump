...
TEST_F(CommandsParserTests, TestDuration)
{
	...
	ASSERT_EQ(parsedCommand.options["duration"].duration, 37 * CommandsParser::second);

	args[2] = "3min";
	parsedCommand = CommandsParser::parse(args, options);
	ASSERT_TRUE(OptionExists(parsedCommand, "duration"));
	ASSERT_EQ(parsedCommand.options["duration"].duration, 3 * CommandsParser::minute);


	args[2] = "5h";
	parsedCommand = CommandsParser::parse(args, options);
	ASSERT_TRUE(OptionExists(parsedCommand, "duration"));
	ASSERT_EQ(parsedCommand.options["duration"].duration, 5 * CommandsParser::hour);

	args[2] = "7d";
	parsedCommand = CommandsParser::parse(args, options);
	ASSERT_TRUE(OptionExists(parsedCommand, "duration"));
	ASSERT_EQ(parsedCommand.options["duration"].duration, 7 * CommandsParser::day);

	args[2] = "9w";
	parsedCommand = CommandsParser::parse(args, options);
	ASSERT_TRUE(OptionExists(parsedCommand, "duration"));
	ASSERT_EQ(parsedCommand.options["duration"].duration, int64_t(9 * 7) * CommandsParser::day);

	args[2] = "11mon";
	parsedCommand = CommandsParser::parse(args, options);
	ASSERT_TRUE(OptionExists(parsedCommand, "duration"));
	ASSERT_EQ(parsedCommand.options["duration"].duration, int64_t(11 * 30) * CommandsParser::day);

	args[2] = "13y";
	parsedCommand = CommandsParser::parse(args, options);
	ASSERT_TRUE(OptionExists(parsedCommand, "duration"));
	ASSERT_EQ(parsedCommand.options["duration"].duration, int64_t(13 * 365) * CommandsParser::day);
}
...
TEST_F(CommandsParserTests, TestTooBigInteger)
{
	struct CommandsParser::ParseOptions options;
	options.optionDefinitions.push_back({ "integer", CommandsParser::OptionDefinition::Type::Integer, "A description" });
	...
	ASSERT_EQ(parsedCommand.command, "!throw");
	ASSERT_TRUE(OptionExists(parsedCommand, "integer"));
	ASSERT_TRUE(parsedCommand.options["integer"].errorMessage.length() > 0);
}
