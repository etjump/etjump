...
void SP_info_null(gentity_t *self) {
  // Gordon: if it has a targetname, let it stick around for a few
  // frames
...
namespace ETJump {
void calcRelativeAngles(gentity_t *self, gentity_t *other, vec3_t origin,
                        vec3_t angles, const int &spawnflags) {
  vec3_t newOrigin{}, newViewAngles{};
  vec3_t newVelocity{}, offset{};
  ...
  VectorNormalize2(self->client->ps.velocity, normalizedVelocity);
  ...
  // calculate the intersection-points of player movement with the 6 planes
  // of the trigger brush - the shortest distance between player and
  // intersection points selects the plane
  // (where did we get into the trigger, from what side?)
  if (normalizedVelocity[0] != 0.0f) {
    VectorMA(self->client->ps.origin,
             (other->r.mins[0] - self->client->ps.origin[0]) /
                 normalizedVelocity[0],
             normalizedVelocity, sPlane[0]);
    VectorMA(self->client->ps.origin,
             (other->r.maxs[0] - self->client->ps.origin[0]) /
                 normalizedVelocity[0],
             normalizedVelocity, sPlane[3]);
    minDistYZ = std::min(VectorDistance(self->client->ps.origin, sPlane[0]),
                         VectorDistance(self->client->ps.origin, sPlane[3]));
  } else {
    minDistYZ = 10000.0f;
  }
  if (normalizedVelocity[1] != 0.0f) {
    VectorMA(self->client->ps.origin,
             (other->r.mins[1] - self->client->ps.origin[1]) /
                 normalizedVelocity[1],
             normalizedVelocity, sPlane[1]);
    VectorMA(self->client->ps.origin,
             (other->r.maxs[1] - self->client->ps.origin[1]) /
                 normalizedVelocity[1],
             normalizedVelocity, sPlane[4]);
    minDistXZ = std::min(VectorDistance(self->client->ps.origin, sPlane[1]),
                         VectorDistance(self->client->ps.origin, sPlane[4]));
  } else {
    minDistXZ = 10000.0f;
  }
  if (normalizedVelocity[2] != 0.0f) {
    VectorMA(self->client->ps.origin,
             (other->r.mins[2] - self->client->ps.origin[2]) /
                 normalizedVelocity[2],
             normalizedVelocity, sPlane[2]);
    VectorMA(self->client->ps.origin,
             (other->r.maxs[2] - self->client->ps.origin[2]) /
                 normalizedVelocity[2],
             normalizedVelocity, sPlane[5]);
    minDistXY = std::min(VectorDistance(self->client->ps.origin, sPlane[2]),
                         VectorDistance(self->client->ps.origin, sPlane[5]));
  } else {
    minDistXY = 10000.0f;
  }
  ...
  VectorCopy(self->client->ps.origin, tempActivator);

  //////////////////////////////////////////////////////////////////////////////
  // PLANE YZ (NORMAL X)
  if (minDistYZ < minDistXZ && minDistYZ < minDistXY) {
    // check +/- direction of trigger
    if (triggerOrigin[0] < self->client->ps.origin[0]) {
      negDir = true;
    }

    // calc viewangles
    VectorCopy(angles, newViewAngles);
    newViewAngles[YAW] += self->client->ps.viewangles[YAW];

    if (adjustPitch) {
      newViewAngles[PITCH] += self->client->ps.viewangles[PITCH];
    }

    // calc velocity
    speed = VectorLength(self->client->ps.velocity);
    vectoangles(self->client->ps.velocity, veloAngles);
  ...
  // PLANE XZ (NORMAL Y)
    ...
    // check +/- direction of trigger
    if (triggerOrigin[1] < self->client->ps.origin[1]) {
      negDir = true;
    }

    // calc viewangles
    VectorCopy(angles, newViewAngles);
    newViewAngles[YAW] += (self->client->ps.viewangles[YAW] - 90.0f);
  ...
  // set velocity
  VectorCopy(newVelocity, self->client->ps.velocity);
  // correct viewangles
  if (negDir) {
...
void teleportPlayer(gentity_t *self, gentity_t *other, const vec3_t origin,
  ...
  vec3_t teleOrigin{}, teleAngles{};

  // copy origin and angles to new variables, to avoid relative angle teleports
  // potentially mutilating the destination origin/angles
  VectorCopy(origin, teleOrigin);
  VectorCopy(angles, teleAngles);

  if (spawnflags & static_cast<int>(TeleporterSpawnflags::Knockback)) {
    self->client->ps.pm_time = 160;
    self->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
  }

  if (spawnflags & static_cast<int>(TeleporterSpawnflags::ResetSpeed)) {
    // we need some speed to make calcRelativeAngles work with
    // this spawnflag, else it doesn't know which trigger side we enter
    VectorSet(self->client->ps.velocity, 0.01f, 0.01f, 0.01f);
  }

  if (spawnflags & static_cast<int>(TeleporterSpawnflags::ConvertSpeed)) {
    const float speed = VectorLength(self->client->ps.velocity);

    AngleVectors(teleAngles, self->client->ps.velocity, nullptr, nullptr);
    VectorScale(self->client->ps.velocity, speed, self->client->ps.velocity);
  } else if (spawnflags &
             (static_cast<int>(TeleporterSpawnflags::RelativeYaw) |
              static_cast<int>(TeleporterSpawnflags::RelativePitchYaw))) {
    calcRelativeAngles(self, other, teleOrigin, teleAngles, spawnflags);
  }

  VectorCopy(teleOrigin, self->client->ps.origin);

  if (!(spawnflags & static_cast<int>(TeleporterSpawnflags::NoZOffset))) {
    self->client->ps.origin[2] += 1;
  }
...
void grabber_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
  ...
  // FIXME FIXME
  // this is buggy.  the trigger brush entity (ent->enemy) does not
  // free. need to fix.
...
==============
grabber_wake
    ent calling this is the bounding box for the grabber, not the grabber ent
itself. the grabber ent is 'ent->parent'
==============
*/
...
void SP_misc_grabber_trap(gentity_t *ent) {
  ...
  // just make an even trigger box around the ent (do properly
  // sized/oriented trigger after it's working)
  if (1) // temp fast trigger
  {
...
"modelscale_vec"	scale multiplier (defaults to 1 1 1, scales each axis as
requested) "skin"			.skin file used to define shaders for
model
"trunk"			diameter of solid core (used for trace visibility and
collision (not ai pathing)) "trunkheight"	height of trunk
...
void SP_misc_gamemodel(gentity_t *ent) {
  ...
  // Gordon: static gamemodels client side only now :D so server can
  // just wave bye-bye
  ...
  if (ent->spawnflags & 2) {
    ...
    // xkan, 11/08/2002 - continue loop animation as long as
    // s.teamNum == 0
    ent->s.teamNum = 0;
  }
  ...
  if (ent->spawnflags & 1) {
    ent->s.apos.trType = TR_INTERPOLATE; // misc_gamemodels (since they have no
                                         // movement) will use type = 0 for
                                         // static models, type = 1 for
                                         // auto-aligning models
  }
  trap_LinkEntity(ent);
}
...
void SP_shooter_mortar(gentity_t *ent) {
  // (SA) TODO: must have a self->target.  Do a check/print if this is
  // not the case.
...
void Fire_Lead_Ext(gentity_t *ent, gentity_t *activator, float spread,
  ...
  if (traceEnt->takedamage && traceEnt->client) {
    ...
    tent->s.otherEntityNum2 =
        activator->s.number; // (SA) store the user id, so the
                             // client can position the tracer
...
void clamp_hweapontofirearc(gentity_t *self, vec3_t dang) {
  ...
  // sanity check the angles again to make sure we don't go passed the
  // harc
...
void aagun_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
               int damage, int mod) {}
...
void mg42_fire(gentity_t *other) {
  ...
  // VectorMA (muzzle, 16, forward, muzzle); // JPW NERVE unnecessary
  // and makes it so close-range enemies get missed

  // Arnout: disabled this as it was causing troubles with murderholes.
  // Why was it there? Maybe for beach to let the mg42 point down and
  // still shoot over teh concrete? FIX: make the HIGH spawnflag
  // actually work
...
void mg42_think(gentity_t *self) {
  ...
  if (owner->client) {
    ...
    if (len < usedist && owner->active && owner->health > 0) {
      ...
      // owner->client->ps.weapHeat[WP_DUMMY_MG42] =
      // self->mg42weapHeat;
  ...
  if (owner->client) {
    ...
    // need this here for players that get killed while on the
    // MG42
...
void mg42_spawn(gentity_t *ent) {
  ...
  if (g_knifeonly.integer != 1) {
    // Need to spawn the base even when no tripod cause the gun
    // itself isn't solid
    base = G_Spawn();
    ...
    if (!(ent->spawnflags & 2)) // no tripod
    {
      ...
      // Arnout: move track and targetname over to these
      // entities for construction system
    ...
    // Arnout: move track and targetname over to these entities
...
/*void miscGunnerEnemyScan(gentity_t *ent, vec3_t angles)
{
    ...
    for (t=g_entities; t<g_entities+level.maxclients; t++) {
        ...
        if (t->health < 0)
            continue;
        if (VectorDistanceSquared(ent->r.currentOrigin, t->r.currentOrigin) >
SQR(ent->radius)) continue; VectorSubtract( t->r.currentOrigin,
ent->r.currentOrigin, v ); vectoangles( v, tang ); if (!AICast_InFieldOfVision(
angles, ent->harc, tang )) continue; if (!AICast_VisibleFromPos(
ent->r.currentOrigin, ent->s.number, t->r.currentOrigin, t->s.number, qfalse ))
            continue;
        // found an enemy
        ent->enemy = t;
        break;
...
/*void miscGunnerThink(gentity_t *ent)
{
    ...
    if (ent->enemy) {
        if (ent->enemy->health <= 0) {
            ent->enemy = NULL;
        } else if (AICast_VisibleFromPos(ent->r.currentOrigin, ent->s.number,
ent->enemy->r.currentOrigin, ent->enemy->s.number, qfalse)) { fire = qtrue;
        }
    }
    ...
    if (!fire) {
        ...
        if (ent->enemy) {
            ...
            // if we have just found our first enemy, release us from the "fixed
tag" angles
            // so we can track them
            if (!gun->s.density) {
                gun->s.density = 1;
                // start facing the same direct, so we don't snap to a different
angle immediately BG_EvaluateTrajectory( &truck->s.apos, level.time,
ent->s.angles ); VectorCopy( gun->s.angles, gun->s.apos.trBase );
...
gun->r.currentAngles ); diff = AngleDifference( dang[i], gun->r.currentAngles[i]
                ...
                if (diff > 0) {
                    dang[i] = AngleMod(gun->r.currentAngles[i] + (yawspeed *
((float)FRAMETIME/1000.0))); } else { dang[i] = AngleMod(gun->r.currentAngles[i]
...
void misc_firetrails_think(gentity_t *ent) {
  ...
  if (!airplane) {
    G_Error("can't find airplane with targetname \"%s\" for "
            "firetrails",
            ent->target);
  }
...
void constructiblemarker_setup(gentity_t *ent) {
  ...
  if (!ent->target_ent) {
    G_Error("'misc_constructiblemarker' has a missing target "
            "'%s'\n",
            ent->target);
  }
...
void landmine_setup(gentity_t *ent) {
  ...
  if (tr.startsolid || tr.fraction == 1.f ||
    ...
    G_Printf("^3WARNING: 'misc_landmine' entity at %.2f %.2f "
             "%.2f doesn't have "
...
void SP_func_fakebrush(gentity_t *ent) {
  // all this values should be already set in G_ParseField but make sure
  // they were really found
  ...
  // rape origin2 and angles2 to save mins and maxs for client
  // prediction
