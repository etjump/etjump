...
void alarmbox_updateparts(gentity_t *ent, qboolean matestoo) {
  ...
  if (matestoo) {
    for (mate = ent->teammaster; mate; mate = mate->teamchain) {
      ...
      if (!(mate->active)) // don't update dead alarm
                           // boxes, they stay dead
      {
        continue;
      }

      if (!(ent->active)) // destroyed, so just turn
                          // teammates off
      {
        mate->s.frame = 0;
      } else {
  ...
  while ((t = G_FindByTargetname(t, ent->target)) != NULL) {
    if (t == ent) {
      ...
      // alarmbox can tell script_trigger about
      // activation (but don't trigger if dying, only
      // activation)
...
void alarmbox_finishspawning(gentity_t *ent) {
  ...
  // make sure they all have the same master (picked arbitrarily.  last
  // spawned)
