...
void G_LogKill(gentity_t *ent, weapon_t weap) {
  ...
  trap_PbStat(ClientNum(ent), "kill",
              va("%d %d %d", ent->client->sess.sessionTeam,
                 ent->client->sess.playerType, weap));
}
...
extern qboolean AddWeaponToPlayer(gclient_t *client, weapon_t weapon, int ammo,
...
void G_AddSkillPoints(gentity_t *ent, skillType_t skill, float points) {
  ...
  G_SetPlayerSkill(ent->client, skill);
  if (oldskill != ent->client->sess.skill[skill]) {
    // TAT - call the new func that encapsulates the skill
    // giving behavior
...
void G_LoseKillSkillPoints(gentity_t *tker, meansOfDeath_t mod, hitRegion_t hr,
  ...
  switch (mod) {
    ...
    case MOD_M7:
      ...
      //			G_DebugAddSkillPoints(
      // attacker,
      // SK_EXPLOSIVES_AND_CONSTRUCTION, 4.f, "dynamite
      // or landmine kill" );
      break;
    ...
    case MOD_AKIMBO_SILENCEDLUGER:
      switch (hr) {
        ...
        default:
          ...
          break; // for weapons that
                 // don't have localized
                 // damage
      }
      break;
...
void G_DebugAddSkillLevel(gentity_t *ent, skillType_t skill) {
  ...
  if (g_debugSkills.integer >= 2 && skillDebugLog != -1) {
    char *s = va("%02d:%02d:%02d : ^%c(SK: %2i XP: %6.2f) %s: %s "
                 "raised in "
                 "skill level to %i.\n",
                 ...
                 ent->client->pers.netname, ent->client->sess.skill[skill]);
    trap_FS_Write(s, strlen(s), skillDebugLog);
  }
}
...
void G_DebugAddSkillPoints(gentity_t *ent, skillType_t skill, float points,
  ...
  trap_SendServerCommand(ent - g_entities,
                         va("sdbg \"^%c(SK: %2i XP: %6.2f) %s: You "
                            "gained %.2fXP, reason: %s.\"\n",
                            COLOR_RED + skill, ent->client->sess.skill[skill],
                            ent->client->sess.skillpoints[skill],
                            skillNames[skill], points, reason));

  trap_RealTime(&ct);

  if (g_debugSkills.integer >= 2 && skillDebugLog != -1) {
    char *s =
        va("%02d:%02d:%02d : ^%c(SK: %2i XP: %6.2f) %s: %s gained "
           "%.2fXP, "
           "reason: %s.\n",
           ct.tm_hour, ct.tm_min, ct.tm_sec, COLOR_RED + skill,
