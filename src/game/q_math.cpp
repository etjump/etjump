...
vec4_t g_color_table[32] = {
    ...
    {0.75f, 0.75f, 0.75f,
     1.0f}, // : - lt.grey      10      // lt grey for names
...
===============
*/
void RotatePointAroundVertex(vec3_t pnt, float rot_x, float rot_y, float rot_z,
                             const vec3_t origin) {
  float tmp[11];
...
================
*/
int AngleNormalize65536(int angle) { return angle & 65535; }
...
__inline __declspec(naked) int BoxOnPlaneSide_fast(vec3_t emins, vec3_t emaxs,
                                                   struct cplane_s *p) {
...
void AxisToAngles(vec3_t axis[3], vec3_t angles) {
  ...
  // get the angle difference between the tmpAxis[2] and axis[2] after
  // they have been reverse-rotated
  ...
  // if the yaw is more than 90 degrees difference, we should adjust the
  // pitch
