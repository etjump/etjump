...
{
	return SystemCall(G_GETTAG, clientNum, tagFileNumber, tagName, orientation) ? qtrue : qfalse;
}
...
{
	return SystemCall(BOTLIB_SETUP);
}

int trap_BotLibShutdown(void)
{
	return SystemCall(BOTLIB_SHUTDOWN);
}
...
{
	return SystemCall(BOTLIB_AAS_INITIALIZED);
}
...
{
	int temp;
	temp = SystemCall(BOTLIB_AAS_TIME);
	return PASSINT(temp);
}
...
{
	return SystemCall(BOTLIB_AAS_TRACE_AREAS, start, end, areas, points, maxareas);
}
...
{
	return SystemCall(BOTLIB_AAS_AREA_TRAVEL_TIME_TO_GOAL_AREA, areanum, origin, goalareanum, travelflags);
}
...
{
	return SystemCall(BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT, move, entnum, origin, presencetype, onground, velocity, cmdmove, cmdframes, maxframes, PASSFLOAT(frametime), stopevent, stopareanum, visualize);
}
...
//	return SystemCall( BOTLIB_AAS_RT_GETHIDEPOS, srcpos, srcnum, srcarea, destpos, destnum, destarea, returnPos );
//}

//int trap_AAS_FindAttackSpotWithinRange(int srcnum, int rangenum, int enemynum, float rangedist, int travelflags, float *outpos) {
//	return SystemCall( BOTLIB_AAS_FINDATTACKSPOTWITHINRANGE, srcnum, rangenum, enemynum, PASSFLOAT(rangedist), travelflags, outpos );
//}
...
{
	return SystemCall(BOTLIB_AAS_NEARESTHIDEAREA, srcnum, origin, areanum, enemynum, enemyorigin, enemyareanum, travelflags, PASSFLOAT(maxdist), distpos);
}

int trap_AAS_ListAreasInRange(vec3_t srcpos, int srcarea, float range, int travelflags, float **outareas, int maxareas)
{
	return SystemCall(BOTLIB_AAS_LISTAREASINRANGE, srcpos, srcarea, PASSFLOAT(range), travelflags, outareas, maxareas);
}

int trap_AAS_AvoidDangerArea(vec3_t srcpos, int srcarea, vec3_t dangerpos, int dangerarea, float range, int travelflags)
{
	return SystemCall(BOTLIB_AAS_AVOIDDANGERAREA, srcpos, srcarea, dangerpos, dangerarea, PASSFLOAT(range), travelflags);
}
...
{
	return SystemCall(BOTLIB_AAS_RETREAT, dangerSpots, dangerSpotCount, srcpos, srcarea, dangerpos, dangerarea, PASSFLOAT(range), PASSFLOAT(dangerRange), travelflags);
}
...
{
	SystemCall(BOTLIB_AAS_RECORDTEAMDEATHAREA, srcpos, srcarea, team, teamCount, travelflags);
}
// done.
...
{
	SystemCall(BOTLIB_EA_GESTURE, client);
}
...
{
	SystemCall(BOTLIB_EA_TALK, client);
}

void trap_EA_Attack(int client)
{
	SystemCall(BOTLIB_EA_ATTACK, client);
}

void trap_EA_Reload(int client)
{
	SystemCall(BOTLIB_EA_RELOAD, client);
}

void trap_EA_Activate(int client)
{
	SystemCall(BOTLIB_EA_USE, client);
}

void trap_EA_Respawn(int client)
{
	SystemCall(BOTLIB_EA_RESPAWN, client);
}

void trap_EA_Jump(int client)
{
	SystemCall(BOTLIB_EA_JUMP, client);
}
...
{
	SystemCall(BOTLIB_EA_CROUCH, client);
}

void trap_EA_Walk(int client)
{
	SystemCall(BOTLIB_EA_WALK, client);
}

void trap_EA_MoveUp(int client)
{
	SystemCall(BOTLIB_EA_MOVE_UP, client);
}

void trap_EA_MoveDown(int client)
{
	SystemCall(BOTLIB_EA_MOVE_DOWN, client);
}
...
{
	SystemCall(BOTLIB_EA_MOVE_BACK, client);
}

void trap_EA_MoveLeft(int client)
{
	SystemCall(BOTLIB_EA_MOVE_LEFT, client);
}

void trap_EA_MoveRight(int client)
{
	SystemCall(BOTLIB_EA_MOVE_RIGHT, client);
}
...
{
	SystemCall(BOTLIB_EA_PRONE, client);
}
...
{
	int temp;
	temp = SystemCall(BOTLIB_AI_CHARACTERISTIC_FLOAT, character, index);
	return PASSINT(temp);
}
...
{
	int temp;
	temp = SystemCall(BOTLIB_AI_CHARACTERISTIC_BFLOAT, character, index, PASSFLOAT(min), PASSFLOAT(max));
	return PASSINT(temp);
}
...
{
	return SystemCall(BOTLIB_AI_CHARACTERISTIC_BINTEGER, character, index, min, max);
}
...
{
	SystemCall(BOTLIB_AI_INITIAL_CHAT, chatstate, type, mcontext, var0, var1, var2, var3, var4, var5, var6, var7);
}
...
{
	return SystemCall(BOTLIB_AI_REPLY_CHAT, chatstate, message, mcontext, vcontext, var0, var1, var2, var3, var4, var5, var6, var7);
}
...
{
	return SystemCall(BOTLIB_AI_CHOOSE_LTG_ITEM, goalstate, origin, inventory, travelflags);
}

int trap_BotChooseNBGItem(int goalstate, vec3_t origin, int *inventory, int travelflags, void /* struct bot_goal_s */ *ltg, float maxtime)
{
	return SystemCall(BOTLIB_AI_CHOOSE_NBG_ITEM, goalstate, origin, inventory, travelflags, ltg, PASSFLOAT(maxtime));
}
...
{
	return SystemCall(BOTLIB_AI_ITEM_GOAL_IN_VIS_BUT_NOT_VISIBLE, viewer, eye, viewangles, goal);
}
...
{
	int temp;
	temp = SystemCall(BOTLIB_AI_AVOID_GOAL_TIME, goalstate, number);
	return PASSINT(temp);
}

void trap_BotInitLevelItems(void)
{
	SystemCall(BOTLIB_AI_INIT_LEVEL_ITEMS);
}
...
{
	return SystemCall(BOTLIB_AI_MOVE_IN_DIRECTION, movestate, dir, PASSFLOAT(speed), type);
}
...
{
	return SystemCall(BOTLIB_AI_MOVEMENT_VIEW_TARGET, movestate, goal, travelflags, PASSFLOAT(lookahead), target);
}

int trap_BotPredictVisiblePosition(vec3_t origin, int areanum, void /* struct bot_goal_s */ *goal, int travelflags, vec3_t target)
{
	return SystemCall(BOTLIB_AI_PREDICT_VISIBLE_POSITION, origin, areanum, goal, travelflags, target);
}
...
{
	return SystemCall(BOTLIB_AI_GENETIC_PARENTS_AND_CHILD_SELECTION, numranks, ranks, parent1, parent2, child);
}
