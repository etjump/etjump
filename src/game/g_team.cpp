...
void QDECL PrintMsg(gentity_t *ent, const char *fmt, ...) {
  ...
  // NOTE: if buffer overflow, it's more likely to corrupt stack and
  // crash than do a proper G_Error?
  va_start(argptr, fmt);
  if (Q_vsnprintf(msg, sizeof(msg), fmt, argptr) >
...
qboolean OnAnyTeam(gentity_t *ent1, gentity_t *ent2) {
  ...
  if (ent1->client->sess.sessionTeam == ent2->client->sess.sessionTeam ||
      (ent1->client->sess.sessionTeam == TEAM_AXIS &&
       ent2->client->sess.sessionTeam == TEAM_ALLIES) ||
      (ent1->client->sess.sessionTeam == TEAM_ALLIES &&
       ent2->client->sess.sessionTeam == TEAM_AXIS)) {
    return qtrue;
  }
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor,
  ...
  // JPW NERVE -- no bonuses for fragging friendlies, penalties scored
  // elsewhere
  ...
  {         //		return; // can't find attacker's flag
    ...
    // ok we have the attackers flag and a pointer to the
    // carrier
    ...
    if ((VectorLengthSquared(v1) < SQR(CTF_TARGET_PROTECT_RADIUS) ||
      ...
      // JPW NERVE FIXME -- don't report flag defense
      // messages, change to gooder message
    ...
    if ((flag->s.frame != WCP_ANIM_NOFLAG) &&
      ...
      if (VectorLengthSquared(v1) < SQR(WOLF_CP_PROTECT_RADIUS)) {
        if (flag->spawnflags & 1) // protected spawnpoint
        {
          ...
          // G_AddExperience( attacker,
          // 0.5f );
        } else {
          AddScore(attacker,
                   WOLF_CP_PROTECT_BONUS); // protected checkpoint
                                           // G_AddExperience( attacker,
                                           // 0.5f );
        }
      }
...
int Team_TouchOurFlag(gentity_t *ent, gentity_t *other, int team) {
  ...
  if (ent->flags & FL_DROPPED_ITEM) {
    ...
    //		te = G_TempEntity( other->s.pos.trBase,
    // EV_GLOBAL_SOUND ); 		te->r.svFlags |= SVF_BROADCAST;
...
int Team_TouchEnemyFlag(gentity_t *ent, gentity_t *other, int team) {
  ...
  if (cl->sess.sessionTeam == TEAM_AXIS) {
    ...
    //		trap_SendServerCommand(-1, va("cp \"Axis have stolen
    //%s!\n\" 2",
    // ent->message));
    ...
    //		trap_SendServerCommand(-1, va("cp \"Allies have stolen
    //%s!\n\"
    // 2", ent->message));
  ...
  if (ent->s.density > 0) {
    return 1; // We have more flags to give out, spawn back
              // quickly
  } else {
    return -1; // Do not respawn this automatically, but do
               // delete it if it was FL_DROPPED
  }
}
...
void Use_Team_InitialSpawnpoint(gentity_t *ent, gentity_t *other,
                                gentity_t *activator) {
  if (ent->spawnflags & 4) {
    ent->spawnflags &= ~4;
  } else {
...
void Use_Team_Spawnpoint(gentity_t *ent, gentity_t *other,
                         gentity_t *activator) {
  if (ent->spawnflags & 2) {
    ent->spawnflags &= ~2;
...
void SP_team_CTF_redplayer(gentity_t *ent) {
  ...
  G_Printf("^1team_ctf_*player entities are now obsolete, please "
           "remove them!\n");
  G_FreeEntity(ent);
  return;
...
// swaps the team
void team_wolf_objective_use(gentity_t *self, gentity_t *other,
                             gentity_t *activator) {
  char cs[MAX_STRING_CHARS];
...
// Fixes ERROR: SP_team_WOLF_objective: exceeded MAX_MULTI_SPAWNTARGETS (16)
// caused by certain maps on linux.
void ResetNumSpawnTargets() { numobjectives = 0; }
...
void objective_Register(gentity_t *self) {
  ...
  if (numobjectives == MAX_MULTI_SPAWNTARGETS) {
    G_Error("SP_team_WOLF_objective: exceeded "
            "MAX_MULTI_SPAWNTARGETS (%d). "
            "numobjectives: (%d)\n",
...
void checkpoint_spawntouch(gentity_t *self, gentity_t *other,
...
void checkpoint_spawntouch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  // If this is the first time it's being touched, and it was the
  // opposing team touching a single-team reinforcement flag... don't do
  // anything.
  ...
  // Arnout - updated this to allow toggling of initial spawnpoints as
  // well, plus now it only toggles spawnflags 2 for spawnpoint entities
  if (self->target) {
    while (1) {
      ...
      if (other->client->sess.sessionTeam == TEAM_AXIS) {
        if (!strcmp(ent->classname, "team_CTF_redspawn")) {
          ...
          /*				} else
             if
             (!strcmp(ent->classname,"team_CTF_redplayer"))
             { ent->spawnflags &= ~4; }
             else if
             (!strcmp(ent->classname,"team_CTF_blueplayer"))
             { ent->spawnflags |= 4;*/
        }
      } else {
...
void SP_team_WOLF_checkpoint(gentity_t *ent) {
  ...
  // Model is user assignable, but it will always try and use the
  // animations for flagpole.md3
  if (ent->model) {
...
// OSP
const char *aTeams[TEAM_NUM_TEAMS] = {"FFA", "^1Axis^7", "^4Allies^7",
                                      "Spectators"};
...
qboolean G_readyMatchState(void) {
  if ((g_doWarmup.integer || level.warmupTime > (level.time + 10 * 1000)) &&
    ...
    if (g_gamestate.integer == GS_WARMUP_COUNTDOWN) {
      AP("cp \"^1COUNTDOWN STOPPED!^7  Back to "
         "warmup...\n\"");
    }
