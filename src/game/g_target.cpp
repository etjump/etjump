...
void target_init_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if (!(self->spawnflags & static_cast<int>(TargetInitSpawnflags::KeepHealth)))
	{
	...
	if (!(self->spawnflags & static_cast<int>(TargetInitSpawnflags::KeepWeapons)))
	{
		auto weapon = activator->client->sess.playerWeapon;

		// restore the ammo we spawned with incase we don't have our weapon anymore
		if (!(COM_BitCheck(activator->client->ps.weapons, weapon)))
		{
			activator->client->ps.ammo[weapon] = activator->client->sess.ammoOnSpawn[weapon];
			activator->client->ps.ammoclip[weapon] = activator->client->sess.ammoclipOnSpawn[weapon];
		}

		memcpy(activator->client->ps.weapons, activator->client->sess.weaponsOnSpawn, sizeof(activator->client->sess.weaponsOnSpawn));
		...
		if (!(COM_BitCheck(activator->client->ps.weapons, activator->client->ps.weapon)))
		{
			activator->client->ps.weapon = weapon;
		}
	}

	if (!(self->spawnflags & static_cast<int>(TargetInitSpawnflags::KeepPortalgun)))
	{
	...
	if (!(self->spawnflags & static_cast<int>(TargetInitSpawnflags::KeepHoldable)))
	{ 
	...
	if (!(self->spawnflags & static_cast<int>(TargetInitSpawnflags::KeepTracker)))
	{
	...
	if (self->spawnflags & static_cast<int>(TargetInitSpawnflags::RemoveStartingWeapons)
		&& !(self->spawnflags & static_cast<int>(TargetInitSpawnflags::KeepWeapons)))
	{
		auto allowedWeapons = std::vector<int>
		{
			WP_NONE,
			WP_KNIFE,
			WP_MEDKIT,
		...
		for (auto i = 0; i < WP_NUM_WEAPONS; i++)
		{
			if (std::find(allowedWeapons.begin(), allowedWeapons.end(), i) != allowedWeapons.end())
			{
				continue;
			}
			COM_BitClear(activator->client->ps.weapons, i);
