...
gentity_t *G_TestEntityPosition(gentity_t *ent) {
  ...
  if (ent->clipmask) {
    //		if ( ent->r.contents == CONTENTS_CORPSE && ent->health
    //<= 0 ) {
    //// Arnout: players waiting to be revived are important
    /// if (
    // ent->r.contents
    //== CONTENTS_CORPSE ) {
    // corpse aren't important
    // G_Damage( ent, NULL, NULL, NULL, NULL, 99999, 0,
    // MOD_CRUSH );
    //			return NULL;
    //		} else {
    mask = ent->clipmask;
  ...
  if (ent->client) {
    ...
    if (!tr.startsolid && ent->client->ps.eFlags & EF_PRONE) {
      ...
      // org[2] = ent->client->ps.origin[2] + 12;
      // // 12 units to play with
      org[2] = ent->client->ps.origin[2] + 24.f; // 12 units to play with

      // VectorSet( point, org[0], org[1], org[2] - 9.6f
      // - 24.f );	// diff between playerlegsMins
      // and playerlegsMaxs z + 24 units to play with
      VectorSet(point, org[0], org[1],
                org[2] - (24.f - 2.4f) - 24.f); // diff between playerlegsMins
                                                // and playerlegsMaxs z + 24
                                                // units to play with
    ...
    pos[2] += 4; // move up a bit - corpses normally got their
                 // origin slightly in the ground
...
qboolean G_TryPushingEntity(gentity_t *check, gentity_t *pusher, vec3_t move,
  ...
  if (check->client) {
    // make sure the client's view rotates when on a rotating
    // mover RF, this is done client-side now ydnar: only do
    // this if player is prone or using set mortar
  ...
  // Arnout, if blocking entity is a player, try to move this player
  // first.
  ...
  // RF, if still not valid, move them around to see if we can find a
  // good spot
...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove,
  ...
  *obstacle = NULL;
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    // check->s.pos.trDelta ) ) { 	continue;	// it's
    // moving
    // }
    ...
    if (check->s.groundEntityNum != pusher->s.number) {
      ...
      // see if the ent's bbox is inside the pusher's
      // final position this does allow a fast moving
      // object to pass through a thin entity...
  ...
  // unlink all to be moved entities so they cannot get stuck in each
  // other
  ...
  for (e = 0; e < moveEntities; e++) {
    ...
    if (G_TryPushingEntity(check, pusher, move, amove)) {
      ...
      // link it in now so nothing else tries to clip
      // into us
    ...
    *obstacle = check;
    ...
    // rain - changed the loop slightly to avoid checking an
    // invalid pointer (do the -1 inside the loop, > instead of
    // >=)
...
void SetMoverState(gentity_t *ent, moverState_t moverState, int time) {
  ...
  switch (moverState) {
    ...
    case MOVER_2TO1: // closing
      ...
      if (ent->closespeed) //----(SA)	handle doors
                           // with different close speeds
      {
    ...
    case MOVER_1TO2ROTATE:             // opening
      VectorClear(ent->s.apos.trBase); // set base to start
                                       // position {0,0,0}

      if (kicked) {
        f = 2000.0 / ent->gDuration; // double speed when
                                     // kicked open
        ent->s.apos.trDuration = ent->gDuration / 2.0;
      } else if (soft) {
        f = 500.0 / ent->gDuration; // 1/2 speed when
                                    // soft opened
  ...
  // RF,
  // added this for bats, but this is safe for all movers, since if they
  // aren't solid, and aren't visible to the client, they don't need to
  // be linked
  //		trap_LinkEntity( ent );

  if (!(ent->r.svFlags & SVF_NOCLIENT) ||
      (ent->r.contents)) // RF, added this for bats, but this is safe
                         // for all movers, since if they aren't solid,
                         // and aren't visible to the client, they don't
                         // need to be linked
  {
    trap_LinkEntity(ent);
    if (ent->key || ent->allowteams) // doors should only block
                                     // when locked and at POS1
    {
      switch (moverState) {
        ...
        case MOVER_POS1:
          ...
          if (ent->allowteams) {
            ...
            if (!blockflags) {
              if (g_cheats.integer) {
                G_Printf("WARNING: %s at %s has allowteams flag, but no "
                         "spawnflags to define which team is allowed\n",
...
void Reached_BinaryMover(gentity_t *ent) {
  ...
  if (ent->moverState == MOVER_1TO2) {
    ...
    // to stop sound from being requested if not in pvs anoying
    // bug
    if (ent->flags & FL_SOFTACTIVATE) {
      G_AddEvent(ent, EV_GENERAL_SOUND, ent->soundSoftendc);
    } else {
  ...
  //	ent->flags &= ~(FL_KICKACTIVATE|FL_SOFTACTIVATE);	// (SA) it
  // was
...
void finishSpawningKeyedMover(gentity_t *ent) {
  ...
  // all ents should be spawned, so it's okay to check for special door
  // triggers now
  ...
  if (ent->key == -2) // the key was not set in the spawn
  {
    if (ent->targetname && findNonAIBrushTargeter(ent)) {
      ent->key = -1; // something is targeting this (other than
                     // a trigger_aidoor) so leave locked
    } else {
      ent->key = 0;
    }
  }
  ...
  if (!(ent->flags & FL_TEAMSLAVE)) {
    ...
    // (SA) is this safe?  is ent->spawnflags & 8 consistant
    // among all keyed ents?
    ...
    // (SA) slaves have been marked as FL_TEAMSLAVE now, so they
    // won't finish their think on their own.  So set keys for
    // teamed doors
    for (slave = ent; slave; slave = slave->teamchain) {
      if (slave == ent) {
        continue;
...
void DoorSetSounds(gentity_t *ent, int doortype, qboolean isRotating) {
  ...
  ent->soundSoftopen = G_SoundIndex(va("sound/movers/doors/door%i_openq.wav",
                                       doortype)); // opening quietly
  ent->soundSoftendo = G_SoundIndex(
      va("sound/movers/doors/door%i_endoq.wav", doortype)); // open quietly
  ent->soundSoftclose = G_SoundIndex(va("sound/movers/doors/door%i_closeq.wav",
                                        doortype)); // closing quietly
  ent->soundSoftendc = G_SoundIndex(va("sound/movers/doors/door%i_endcq.wav",
                                       doortype)); // closed quietly
}
...
void SP_func_secret(gentity_t *ent) {
  ...
  if (G_SpawnInt("key", "", &key)) // if door has a key entered, set it
  {
    ...
    ent->key = -1; // otherwise, set the key when this ent
                   // finishes spawning
  }
  // if the key is invalid, set the key in the finishSpawning routine
...
void Think_SetupTrainTargets(gentity_t *ent) {
  ...
  for (path = ent->nextTrain; !path->nextTrain; path = next) {
    ...
    // there may also be other targets that get fired when the
    // corner is reached
    next = NULL;
    do {
      next = G_FindByTargetname(next, path->target);
      if (!next) {
        G_Printf("Train corner at %s without a "
                 "target path_corner\n",
                 vtos(path->s.origin));
        return;
      }
...
void Static_Pain(gentity_t *ent, gentity_t *attacker, int damage,
  ...
  if (ent->spawnflags & 8) {
    ...
    nextActivation =
        ent->wait + (ent->delay < FRAMETIME ? FRAMETIME : ent->delay);
  } else {
    // default behavior
...
void G_BlockThink(gentity_t *ent) { ent->nextthink = level.time + FRAMETIME; }
...
void SP_func_pendulum(gentity_t *ent) {
  ...
  freq = 1 / (M_PI * 2) * sqrt(G_GRAVITY / (3 * length));
...
void SP_func_door_rotating(gentity_t *ent) {
  ...
  {
    G_Error("Too many axis marked in func_door_rotating "
            "entity.  Only choose "
            "one axis of rotation. (defaulting to standard "
            "door rotation)");
    VectorClear(ent->rotate);
    ent->rotate[1] = 1;
  }
...
void target_effect(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  trap_SetConfigstring(CS_TARGETEFFECT,
                       self->dl_shader); //----(SA)	allow shader to
                                         // be set from entity

  // (SA) this should match the values from func_explosive
  tent->s.frame = self->key; // pass the type to the client ("glass",
                             // "wood", "metal", "gibs", "brick",
                             // "stone", "fabric", 0, 1, 2, 3, 4, 5, 6)
...
void SP_target_effect(gentity_t *ent) {
  ...
  if (ent->dl_shader) {
    G_Printf("^1Warning: This feature needs to be cleaned up "
             "from original "
             "wolf since you are using it, go poke Gordon "
             "about it\n");
  }
}
...
==============
BecomeExplosion
    nuke the original entity and create all the debris entities that need to be
synced to clients
==============
*/
...
void func_explosive_explode(gentity_t *self, gentity_t *inflictor,
  ...
  self->takedamage = qfalse; // don't allow anything try to hurt me now
                             // that i'm exploding
  ...
  self->s.frame = self->key;       // pass the type to the client ("glass",
                                   // "wood", "metal", "gibs", "brick",
                                   // "stone", "fabric", 0, 1, 2, 3, 4, 5, 6)

  if (self->damage) {
    //		G_RadiusDamage(self->s.origin, self,
    // self->damage,
  ...
  if (self->target) {
    ...
    // since the explosive might need to fire the target rather
    // than aim at it, only aim at 'info_notnull' ents
    while (1) {
  ...
  // if a valid target entity was not found, check for a specified
  // 'angle' for the explosion direction
  if (!tent) {
    if (self->s.angles[1]) {
      // up
  ...
  if (G_GetWeaponClassForMOD(static_cast<meansOfDeath_t>(mod)) >=
      self->constructibleStats.weaponclass) {
    G_AddKillSkillPointsForDestruction(
        attacker, static_cast<meansOfDeath_t>(mod), &self->constructibleStats);
  }
}
...
void target_explosion_use(gentity_t *self, gentity_t *other,
                          gentity_t *attacker) {
  vec3_t dir = {0, 0, 1};
  gentity_t *tent = NULL;
    ...
    if (Q_stricmp(s, "nosound")) {
      ...
      ent->s.dl_intensity = -1;
       ...
       switch(ent->key) {
           case 0:	// "wood"
               ent->s.dl_intensity =
   G_SoundIndex("sound/world/boardbreak.wav"); break; case 1:	//
   "glass" ent->s.dl_intensity =
   G_SoundIndex("sound/world/glassbreak_01.wav"); break; case 2:
   // "metal" ent->s.dl_intensity =
   G_SoundIndex("sound/world/metalbreak.wav"); break; case 3:	//
   "gibs" ent->s.dl_intensity =
   G_SoundIndex("sound/player/gibsplit1.wav"); break; case 4:	//
   "brick" ent->s.dl_intensity =
   G_SoundIndex("sound/world/debris1.wav"); break; case 5:	//
   "stone" ent->s.dl_intensity =
...
void use_invisible_user(gentity_t *ent, gentity_t *other,
  ...
  if (other->client && ent->spawnflags & startOff) {
    ...
    // know if it's
    // wrong.  I need someone to tell me what a test map is for
    // this (I'll ask Dan tomorrow) not usable by player. turned
    // off.
    auto te = ETJump::soundEvent(noiseOrigin, EV_GENERAL_SOUND_VOLUME,
                                 ent->soundPos1);
...
void G_Activate(gentity_t *ent, gentity_t *activator) {
  if ((ent->s.apos.trType == TR_STATIONARY &&
       ent->s.pos.trType == TR_STATIONARY) &&
      ent->active == qfalse) {
    // trigger the ent if possible, if not, then we'll just wait
    // at the marker until it opens, which could be never(!?)
...
void func_constructible_spawn(gentity_t *self, gentity_t *other,
                              gentity_t *activator);
...
==============
*/
void func_constructible_use(gentity_t *self, gentity_t *other,
                            gentity_t *activator) {
  self->s.angles2[0] = 0;
  self->grenadeFired = 0;
  self->s.modelindex = 0;
  ...
  if (!(self->spawnflags & CONSTRUCTIBLE_BLOCK_PATHS_WHEN_BUILD)) {
    ...
    if (!self->count2) {
      ...
      trap_SetBrushModel(self,
                         va("*%i",
                            self->conbmodels[self->count2 - 1])); // set the
                                                                  // final stage
    }
    trap_LinkEntity(self);
    // ...and restore
...
==============
*/
void func_constructible_explode(gentity_t *self, gentity_t *inflictor,
                                gentity_t *attacker, int damage, int mod) {
  if (self->desstages) {
    if (self->grenadeFired > 1) {
      ...
      if (self->s.angles2[1]) {
        ...
        // relink the objective info to get our
        // indicator back
        if (self->parent) {
          trap_LinkEntity(self->parent);
          // if( self->s.angles2[1] ) {
          if (self->s.angles2[1]) {
            self->s.angles2[1] = 0; // Think_SetupObjectiveInfo
                                    // needs it
            Think_SetupObjectiveInfo(self->parent);
          }
          //}
      ...
      for (e = 0; e < listedEntities; e++) {
        ...
        // ignore everything but items, players
        // and missiles (grenades too)
        if (check->s.eType != ET_MISSILE && check->s.eType != ET_ITEM &&
            check->s.eType != ET_PLAYER && !check->physicsObject) {
          continue;
        }
        ...
        if (block != self) {
          // the entity is blocked by
          // another entity - that block
        ...
        if (check->client || check->s.eType == ET_CORPSE) {
          ...
          // entity, one that we can't
          // just simply kill (basically
          // flags)
          Team_DroppedFlagThink(check);
        } else {
          // remove the landmine from
          // both teamlists
          if (check->s.eType == ET_MISSILE &&
      ...
      if (G_GetWeaponClassForMOD(static_cast<meansOfDeath_t>(mod)) >=
          self->constructibleStats.weaponclass) {
        G_AddKillSkillPointsForDestruction(attacker,
                                           static_cast<meansOfDeath_t>(mod),
      ...
      // unlink
      G_UseEntity(self, inflictor,
                  attacker); // this will unlink (call
                             // func_constructible_use), if
    ...
    G_UseEntity(self, inflictor,
                attacker); // this will unlink (call
                           // func_constructible_use), if another
                           // function is used something is VERY wrong
  }
}
...
void func_constructible_underconstructionthink(gentity_t *ent) {
  if (level.time - ent->lastHintCheckTime >= CONSTRUCT_PREDECAY_TIME) {
    // ent->s.angles2[0] -=
    // 0.5f*(255.f/(ent->wait/(float)FRAMETIME));
    ...
    if (ent->s.angles2[0] < 5) {
      ...
      ent->lastHintCheckTime = level.time; // don't allow building again for
                                           // a lil while
      return;
    }
  }
...
void func_constructiblespawn(gentity_t *ent) {
  ...
  if (ent->constages) {
    ...
    if (ent->count2 && ent->desstages) {
      ...
      for (target_ptr = ptr = ent->desstages; *ptr; ptr++) {
        if (*ptr == ';') {
          ...
          if ((bmodel_ent = G_FindByTargetname(NULL, buf)) != NULL) {
            ...
            if (Q_stricmp(bmodel_ent->classname, "func_"
                                                 "brushmode"
                                                 "l")) {
              G_Error("dess"
                      "tage"
      ...
      if (numDesStages != ent->count2 - 1) {
        G_Error("'func_constructible' has %i "
                "entries in the desstages and %i "
  ...
  if (!(ent->spawnflags & CONSTRUCTIBLE_START_BUILT)) // if not start build
  {
    ...
    // RF, first we must temporarily "build" it so we can
    // disable the areas that it touches
    if (!ent->count2) {
      trap_SetBrushModel(ent, ent->model);
    } else {
      trap_SetBrushModel(
          ent, va("*%i", ent->conbmodels[ent->count2 - 1])); // set the
                                                             // final stage
    }
    ...
    if (!(ent->spawnflags & CONSTRUCTIBLE_INVULNERABLE)) {
      ...
      // if dynamite-able, create a 'destructable'
...
void SP_func_constructible(gentity_t *ent) {
  ...
  if (ent->spawnflags & AXIS_CONSTRUCTIBLE) {
    ...
    G_Error("'func_constructible' does not have a team that "
            "can build it\n");
  }
  ...
  G_SpawnInt( "health", "100", &health ); // default low enough to get
  killed by dynamite ent->sound1to2 = ent->health = health;*/
