...
void sparks_angles_think(gentity_t *ent) {
  ...
  if (ent->target) {
    target = G_FindByTargetname(nullptr, ent->target);
  }
...
void SP_props_sparks(gentity_t *ent) {
  // (SA) don't use in multiplayer right now since it makes decyphering
  // net messages almost impossible

  // Not implemented in ET, spawn function kept for compatibility reasons
...
void InitProp(gentity_t *ent) {
  ...
  // if the "loopsound" key is set, use a constant looping sound when
  // moving
...
void props_radio_dieSEVEN(gentity_t *ent, gentity_t *inflictor,
  ...
  for (i = 0; i < 20; i++) Spawn_Shard(ent, inflictor, 1, ent->count);
...
void Just_Got_Thrown(gentity_t *self) {
  ...
  self->s.dmgFlags = HINT_CHAIR; // so client knows what kind of mover
                                 // it is for cursorhints
...
//		AICast_AudibleEvent( self->s.number, self->r.currentOrigin, 384 );
  ...
  if (level.time > self->random && has_moved) {
    // RF, alert AI of sound event
    //		AICast_AudibleEvent( self->s.number,
    // self->r.currentOrigin, 384
    //);
...
void Props_Chair_Skyboxtouch(gentity_t *ent) { ent->think = G_FreeEntity; }
...
void SP_Props_Chair(gentity_t *ent) {
  ...
  ent->r.svFlags = 0;
  ent->s.eType = ET_MOVER;
  ent->s.dmgFlags = HINT_CHAIR; // so client knows what kind of mover it
                                // is for cursorhints
...
void SP_Props_ChairHiback(gentity_t *ent) {
  ...
  // "models/furniture/chair/hiback.md3" );
...
void Props_Barrel_Animate(gentity_t *ent) {
  ...
  if (!(ent->spawnflags & 1)) {
    ratio = 2.5;
    VectorSubtract(ent->r.currentOrigin, ent->enemy->r.currentOrigin, v);
    moveit(ent, vectoyaw(v), (ent->delay * ratio * FRAMETIME) * .001f);
  }
}
...
void SP_props_snowGenerator(gentity_t *ent) {
  ...
  if (!ent->target) {
    ...
    if (!target) {
      G_Printf("error snowGenerator at loc %s does "
               "cant find target %s\n",
...
inline constexpr int LOCKER_ANIM_DEATHEND = 11;
...
void init_locker(gentity_t *ent) {
  ...
  ent->use = props_locker_use; // trying it rattles the lock (could also
                               // allow 'waking' from trigger)
  ent->pain = props_locker_pain;

  // drop origin down 8 so the designer can put the box entity on the
  // floor rather than /in/ the floor remove if you get a new model from
  // jason w/ the origin moved up 8
  ent->s.origin[2] -= 8;
...
void props_locker_mass(gentity_t *ent) {
  ...
  trap_SetConfigstring(CS_TARGETEFFECT,
                       ent->dl_shader); //----(SA)	allow shader to
                                        // be set from entity
...
void props_locker_death(gentity_t *ent, gentity_t *inflictor,
  ...
  trap_UnlinkEntity(ent);
  ent->r.maxs[2] = 11; // (SA) make the dead bb half height so the item
                       // can look like it's sitting inside
...
void SP_props_footlocker(gentity_t *self) {
  ...
  // NOTE:	Non axis-aligned orientation not allowed.  It will work,
  // but
  //			the bounding box will not exactly match the
  // model.
...
void props_flamethrower_think(gentity_t *ent) {
  ...
  if (ent->spawnflags & 1) // tracking
  {
    ...
    if (!target) {
      ...
      // TAT - we want the vector going the other way
      // for the flame
  ...
  if ((ent->timestamp + ent->duration) > level.time) {
    ...
    //		we actual need to create an entity that is the fire
    // and will do
    // damage
...
void props_flamethrower_init(gentity_t *ent) {
  ...
  if (!target) {
    ...
    VectorCopy(angles, ent->s.apos.trBase);
    VectorCopy(angles,
               ent->s.angles); // RF, added to fix weird release
                               // build issues
  }
