...
void CG_GenerateTracemap(void) {
  ...
  if (!cg.mapcoordsValid) {
    CG_Printf("Need valid mapcoords in the worldspawn to be "
              "able to generate a "
              "tracemap.\n");
    return;
  }
  ...
  for (i = 0; i < TRACEMAP_SIZE; i++) {
    ...
    for (j = 0; j < TRACEMAP_SIZE; j++) {
      ...
      // Find ground
      while (1) {
        // Perform traces up to the sky,
        // repeating at a higher start height if
        ...
        CG_Trace(&tr, start, NULL, NULL, end, ENTITYNUM_NONE,
                 (MASK_SOLID | MASK_WATER));
        tracecount++;
        if (tr.startsolid) // Stuck in
                           // something, skip
                           // over it.
        {
          start[2] -= 64;
        } else if (tr.fraction == 1) // Didn't hit anything,
                                     // we're (probably)
                                     // outside the world
        {
          tracemap.ground[j][i] = MIN_WORLD_HEIGHT;
  ...
  for (i = 0; i < TRACEMAP_SIZE; i++) {
    ...
    for (j = 0; j < TRACEMAP_SIZE; j++) {
      ...
      if (start[2] == MIN_WORLD_HEIGHT) {
        ...
        // Find sky
        while (1) {
          // Perform traces up to the
          // sky, repeating at a higher
          // start height if we start
          // inside a solid.
          ...
          if (tr.startsolid) // Stuck in
                             ...
                             // skip
                             // over it.
          {                  // can happen, tr.endpos
                             // still is valid even if
                             // we're starting in a
            // solid but trace
            // out of it hitting
            // the next surface
                                       ...
                                       // world
          {
            tracemap.sky[j][i] = MAX_WORLD_HEIGHT;
            break;
          } else if (tr.surfaceFlags & SURF_SKY) // Hit sky, this
                                                 // is where we
                                                 // start.
          {
            tracemap.sky[j][i] = tr.endpos[2];
  ...
  // More groundtrace, find ceilings for areas where we don't have
  // ground
    ...
    for (j = 0; j < TRACEMAP_SIZE; j++) {
      ...
      if (tracemap.sky[j][i] == MAX_WORLD_HEIGHT &&
        ...
        CG_Trace(&tr, start, NULL, NULL, end, ENTITYNUM_NONE,
                 MASK_SOLID | MASK_WATER);
        tracecount++;
        if (tr.fraction == 1) // Didn't hit anything, we're
  ...
  for (i = 0; i < TRACEMAP_SIZE; i++) {
    for (j = 0; j < TRACEMAP_SIZE; j++) {
      ...
      // rain - hard clamp because *min and *max are
      // rounded :(
  ...
  i = 0;
  trap_FS_Write(&i, sizeof(i),
                f); // developer directory offset, 4 bytes
...
qboolean BG_LoadTraceMap(char *rawmapname, vec2_t world_mins,
  ...
  if (trap_FS_FOpenFile(va("maps/%s_tracemap.tga", Q_strlwr(rawmapname)), &f,
    ...
    for (i = 0; i < TRACEMAP_SIZE; i++) {
      ...
      /*for( j = 0; j < TRACEMAP_SIZE; j++ ) {
          if( i == 0 && j < 6 ) {
              // abuse first six pixels for our extended
      data switch( j ) { case 0:	trap_FS_Read(
      &ground_min, sizeof(ground_min), f
      ); break; case 1: trap_FS_Read( &ground_max,
      sizeof(ground_max), f ); break; case 2:
      trap_FS_Read( &skyground_min,
      sizeof(skyground_min), f ); break; case 3:
      trap_FS_Read( &skyground_max,
      sizeof(skyground_max), f ); break; case 4:
      trap_FS_Read( &sky_min, sizeof(sky_min), f );
      break; case 5: trap_FS_Read( &sky_max,
      sizeof(sky_max), f ); break;
              }
              tracemap.sky[TRACEMAP_SIZE - 1 - i][j] =
      MAX_WORLD_HEIGHT; tracemap.skyground[TRACEMAP_SIZE
      - 1 - i][j] = MAX_WORLD_HEIGHT;
              tracemap.ground[TRACEMAP_SIZE - 1 - i][j]
      = MIN_WORLD_HEIGHT; continue;
          }

          trap_FS_Read( &datablock, sizeof(datablock), f
      );	// b g r a tracemap.sky[TRACEMAP_SIZE -
      1 - i][j] = (float)datablock[0];	//
      FIXME: swap if( tracemap.sky[TRACEMAP_SIZE - 1 -
      i][j] == 0 ) tracemap.sky[TRACEMAP_SIZE - 1 -
      i][j] = MAX_WORLD_HEIGHT;
      ...
      MAX_WORLD_HEIGHT;
          ...
          //trap_FS_Read( &data, sizeof(data), f );
      // r tracemap.ground[TRACEMAP_SIZE - 1 - i][j] =
      (float)datablock[2];
      // FIXME: swap if( tracemap.ground[TRACEMAP_SIZE -
      1 - i][j] == 0 ) tracemap.ground[TRACEMAP_SIZE - 1
      - i][j] = MIN_WORLD_HEIGHT;

          //trap_FS_Read( &data, sizeof(data), f ); // a
          if( datablock[3] == 0 ) {
              // just in case
              tracemap.skyground[TRACEMAP_SIZE - 1 -
      i][j] = MAX_WORLD_HEIGHT;
              tracemap.ground[TRACEMAP_SIZE - 1 - i][j]
      = MIN_WORLD_HEIGHT;
          }
    ...
    if (ground_max - ground_min == 0) {
      ...
      // rain - scalefactor 254 to compensate for broken
      // etmain behavior
...
float BG_GetSkyHeightAtPoint(vec3_t pos) {
  ...
  i = static_cast<int>((point[0] - tracemap.world_mins[0]) *
                       one_over_mapgrid_factor[0]);
  j = static_cast<int>((point[1] - tracemap.world_mins[1]) *
                       one_over_mapgrid_factor[1]);
