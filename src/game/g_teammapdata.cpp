...
void G_UpdateTeamMapData(void) {
  ...
  for (i = 0, ent = g_entities; i < level.num_entities; i++, ent++) {
    if (!ent->inuse) {
      //			mapEntityData[0][i].valid
      //= qfalse; 			mapEntityData[1][i].valid = qfalse;
      continue;
    }
      ...
      if (ent->health > 0) {
        ...
        if (ent->client->ps.eFlags & EF_ZOOMING) {
          ...
          /*					if(ent->client->landmineSpotted
             ...
             += 10;
                                  if(ent->client->landmineSpotted->count2
             >= 250) { int k;
             ent->client->landmineSpotted->count2
             = 250;
             ...
             { trap_SendServerCommand(
             k, va( "tt
             \"LANDMINES SPOTTED <STOP>
             CHECK COMMAND MAP FOR
             DETAILS <STOP>\"\n" ));
                                          }
                                      }
                                      trap_SendServerCommand(
          ...
          /*					{
                                  //
             find any landmines that
             don't actually exist
             anymore, see if the covert
             ops can see the spot and if
             so - wipe them
             ...
             *teamList =
             ...
             &mapEntityData[1];
                                  ...
                                  mEnt =
             teamList->activeMapEntityData.next;
                                  while(
             mEnt && mEnt !=
             &teamList->activeMapEntityData
             ) { if( mEnt->type ==
             ME_LANDMINE && mEnt->entNum
             == -1 ) { if(
             G_VisibleFromBinoculars(
             ent, NULL, mEnt->org ) ) {
             mEnt = G_FreeMapEntityData(
             teamList, mEnt );

                                              trap_SendServerCommand(
             ent-g_entities, "print
             \"Old Landmine Cleared\n\""
             ); AddScore( ent, 1 );
             continue;
                                          }
                                      }
