...
void Cmd_Load_f(gentity_t *ent)
{
	...
	if (!g_save.integer)
	{
		CP("print \"^3Load ^7is not enabled on this server.\n\"");
		return;
	}
	...
	{
		trap_Argv(1, cmd, sizeof(cmd));
		if ((posNum = atoi(cmd)) < 0 || posNum >= MAX_SAVE_POSITIONS)
		{
	...
	{
		CP("print \"^7usage: ^3load ^7[position]\n\"");
		return;
	}
	...
	if (ent->client->sess.sessionTeam == TEAM_ALLIES)
		pos = ent->client->sess.allies_save_pos + posNum;
	else
		pos = ent->client->sess.axis_save_pos + posNum;
	if (pos->isValid)
	{
...
void Cmd_Save_f(gentity_t *ent)
{
	...
	else if (argc == 2)
	{
		...
		if ((posNum = atoi(cmd)) < 0 || posNum >= MAX_SAVE_POSITIONS)
		{
			CP("print \"^7Invalid position!\n\"");
			return;
		}
	}
	else
	{
		CP("print \"^7Usage: ^3save ^7[position]\n\"");
		return;
	}

	if (ent->client->sess.sessionTeam == TEAM_SPECTATOR)
	{
		CP("cp \"^7You can not ^3save ^7as a spectator!\n\"");
		return;
	}
