...
#ifdef YDNAR_DECAL_MARKS
{
  ...
  for (i = 0; i < 3; i++) {
    /* old */
    //%	points[ 0 ][ i ] = pushedOrigin[ i ] - radius * axis[ 1
    //][ i ] - radius
    //* axis[ 2 ][ i ]; %	points[ 1 ][ i ] = pushedOrigin[ i ]
    //+ radius *
    // axis[ 1 ][ i ] - radius * axis[ 2 ][ i ]; %	points[ 2 ][
    // i ] = pushedOrigin[ i ] + radius * axis[ 1 ][ i ] +
    // radius * axis[ 2 ][ i ]; % points[ 3 ][ i ] =
    // pushedOrigin[ i ] - radius * axis[ 1 ][ i ] + radius *
    // axis[ 2 ][ i ];
...
#else
{
  ...
  vec5_t markPoints[MAX_MARK_POINTS]; // Ridah, made it vec5_t so it
                                      // includes S/T
  vec3_t projection;
  ...
  for (i = 0, mf = markFragments; i < numFragments; i++, mf++) {
    ...
    // if it is a temporary (shadow) mark, add it immediately
    // and forget about it
    if (temporary) {
