...
static void CG_CheckAmmo(const playerState_t *ps) {
  ...
  if (!ps->weapons[0] && !ps->weapons[1]) // (SA) we start out with no weapons,
                                          // so don't make a click on startup
  {
    return;
  }
  ...
  for (i = 0; i < WP_NUM_WEAPONS; i++) {
    ...
    switch (i) {
      ...
      default:
        total +=
            ps->ammo[BG_FindAmmoForWeapon(static_cast<weapon_t>(i))] * 1000;
        break;
        //			default:
        //				total +=
        // ps->ammo[BG_FindAmmoForWeapon(static_cast<weapon_t>(i))]
        // * 200; break;
    }
...
void CG_DamageFeedback(int yawByte, int pitchByte, int damage) {
  ...
  if (slot == MAX_VIEWDAMAGE) {
    return; // no free slots, never override or splats will
            // suddenly disappear
  }
  vd = &cg.viewDamage[slot];

  // if yaw and pitch are both 255, make the damage always centered
  // (falling, etc)
...
void CG_Respawn(qboolean revived) {
  ...
  if (!revived && cgs.clientinfo[cg.clientNum].team != oldTeam) {
    ...
    if (oldTeam == TEAM_SPECTATOR) {
      ETJump::cgame.hudData.spectatorInfo->clearData();
    }
...
void CG_CheckPlayerstateEvents_wolf(playerState_t *ps, playerState_t *ops) {
  ...
  /*
     ...
     ) { cent = &cg_entities[ ps->clientNum ]; cent->currentState.event
     = ps->externalEvent; cent->currentState.eventParm =
     ps->externalEventParm; CG_EntityEvent( cent, cent->lerpOrigin );
      }
  */
...
void CG_CheckPlayerstateEvents(playerState_t *ps, playerState_t *ops) {
  ...
  for (i = ps->eventSequence - MAX_EVENTS; i < ps->eventSequence; i++) {
    ...
    if (i >= ops->eventSequence
        // or the server told us to play another event instead
        // of a predicted event we already issued or something
        // the server told us changed our prediction causing a
        // different event
...
void CG_CheckChangedPredictableEvents(playerState_t *ps) {
  ...
  for (i = ps->eventSequence - MAX_EVENTS; i < ps->eventSequence; i++) {
    ...
    if (i > cg.eventSequence - MAX_PREDICTED_EVENTS) {
      // if the new playerstate event is different from
      // a previously predicted one
...
void CG_TransitionPlayerState(playerState_t *ps, playerState_t *ops) {
  ...
  if (ps->persistant[PERS_SPAWN_COUNT] != ops->persistant[PERS_SPAWN_COUNT]) {
    qboolean wasRevived =
        ps->persistant[PERS_REVIVE_COUNT] != ops->persistant[PERS_REVIVE_COUNT]
            ? qtrue
            : qfalse;
  ...
  if (ps->eFlags & EF_PRONE_MOVING) {
    ...
    if (!(ops->eFlags & EF_PRONE_MOVING)) {
      // ydnar: this screws up auto-switching when
      // dynamite planted or grenade thrown/out of ammo
      //%	CG_FinishWeaponChange( cg.weaponSelect,
      // ps->nextWeapon );
