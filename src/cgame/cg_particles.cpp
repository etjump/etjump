...
void CG_AddParticleToScene(cparticle_t *p, vec3_t org, float alpha) {
  ...
  if (p->type == P_WEATHER || p->type == P_WEATHER_TURBULENT ||
    ...
    // Ridah, had to do this or MAX_POLYS is being exceeded in
    // village1.bsp
    ...
    verts[3].modulate[3] = 255;
             ...
             p->type == P_SMOKE_IMPACT) { // create a front rotating
                                          // facing polygon
  ...
  else if (p->type == P_ANIM || p->type == P_DLIGHT_ANIM) // ydnar
  {
    ...
    if (ratio >= 1.0) {
      ...
      // we'll walk out of bounds when j is calculated
      // below
      ratio = 0.0001;
    }
  ...
  if (!p->pshader) {
    // (SA) temp commented out for DM again.  FIXME: TODO: this
    // needs to be addressed
    //		CG_Printf ("CG_AddParticleToScene type %d p->pshader
    //== ZERO\n",
    // p->type);
    return;
  }
...
void CG_ParticleExplosion(const char *animStr, vec3_t origin, vec3_t vel,
  ...
  p->width = sizeStart;
  p->height =
      sizeStart * shaderAnimSTRatio[anim]; // for sprites that are stretch
                                           // in either direction
...
void CG_ParticleImpactSmokePuffExtended(qhandle_t pshader, vec3_t origin,
                                        int lifetime, int vel, int acc,
                                        int maxroll, float alpha, float size) {
  cparticle_t *p;
  ...
  // xkan, 1/10/2003 - changed calculation to prevent division by 0 for
  // small size
...
void CG_Particle_OilParticle(qhandle_t pshader, vec3_t origin, vec3_t dir,
                             int ptime, int snum) // snum is parent ent number?
{
  cparticle_t *p;
...
void CG_ParticleBloodCloudZombie(centity_t *cent, vec3_t origin, vec3_t dir) {
  ...
  for (i = 0; i < dist; i++) {
    ...
    // RF, stay around for long enough to expand and dissipate
    // naturally
    if (length) {
