...
void CG_BloodTrail(localEntity_t *le) {
  ...
  // rain - need to check FLT_EPSILON because floating-point math sucks
  // <3
  if (vl < FLT_EPSILON) {
    return;
  }
  ...
  // zinx - check against <= 0 instead of == 0, because it can still
  // wrap; (3000 / (FLT_EPSILON*11.7f)) < 0
  if (step <= 0) {
    return;
  }
#endif
  ...
  for (; t <= t2; t += step) {
    ...
    CG_Particle_Bleed(cgs.media.smokePuffShader, newOrigin, vec3_origin, 0,
                      500 + rand() % 200);
#else
    // Ridah, blood trail using trail code (should be faster
    // since we don't have to spawn as many)
...
void CG_ReflectVelocity(localEntity_t *le, trace_t *trace) {
  ...
  if (le->leMarkType == LEMT_BLOOD && trace->startsolid) {
    ...
    // if (cent && cent->currentState.apos.trType !=
    // TR_STATIONARY)
...
void CG_AddFragment(localEntity_t *le) {
  ...
  if (le->leFlags & LEF_SMOKING) {
    ...
    //	TODO: FIXME: this is not quite right, because it'll
    // become fps dependant
    //- in a bad way. 		the slower the fps, the /more/ smoke
    // there'll be, probably driving the fps lower.
    if (!(rand() % 5)) {
  ...
  if (le->pos.trType == TR_STATIONARY) {
    ...
    // trace a line from previous position down, to see if I
    ...
    if (trace.fraction == 1.0) // it's clear, start moving again
    {
      ...
      le->pos.trType = TR_GRAVITY; // nothing below me, start falling
                                   // again
    } else {
      return;
    }
...
void CG_AddSparkElements(localEntity_t *le) {
  ...
  while (1) {
    ...
    // if stuck, kill it
    if (trace.startsolid) {
      // HACK, some walls screw up, so just pass through
      // if starting in a solid
    ...
    */
    ...
    // if it is in a nodrop zone, remove it
    // this keeps gibs from waiting at the bottom of pits of
    // death and floating levels for some reason SFM1.BSP is one
    // big NODROP zone
    ...
    if (trace.fraction < 1.0) {
      ...
      return;
      /*
                  // reflect the velocity on the trace
         plane CG_ReflectVelocity( le, &trace );
                  // the intersection is a fraction of
         the frametime le->pos.trTime = (int)time;
      */
    }
...
void CG_AddDebrisElements(localEntity_t *le) {
  ...
  for (t = le->lastTrailTime + step; t < cg.time; t += step) {
    ...
    // if it is in a nodrop zone, remove it
    // this keeps gibs from waiting at the bottom of pits of
    // death and floating levels
    //		if ( CG_PointContents( trace.endpos, 0 ) &
    // CONTENTS_NODROP ) { 			CG_FreeLocalEntity( le );
    // return;
    //		}
...
void CG_AddShrapnel(localEntity_t *le) {
  ...
  if (le->pos.trType == TR_STATIONARY) {
    ...
    if (t < SINK_TIME) {
      // we must use an explicit lighting origin,
      // otherwise the lighting would be lost as soon as
      // the origin went into the ground
...
static void CG_AddExplosion(localEntity_t *ex) {
  ...
  if (ex->light || 1) {
    ...
    //%	trap_R_AddLightToScene(ent->origin, light,
    // ex->lightColor[0],
...
static void CG_AddSpriteExplosion(localEntity_t *le) {
  ...
  if (le->light || 1) {
    ...
    // Ridah, modified this so the light fades out rather than
    // shrinking
    /*
    light = (float)( cg.time - le->startTime ) / ( le->endTime -
    le->startTime
    ); if ( light < 0.5 ) { light = 1.0; } else { light = 1.0 -
    ( light - 0.5 )
    * 2;
    }
    ...
    */
    ...
    //%	trap_R_AddLightToScene(re.origin, le->light,
    // light*le->lightColor[0],
