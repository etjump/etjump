...
typedef struct {
  ...
  entityState_t entities[MAX_ENTITIES_IN_SNAPSHOT]; // at the time of
                                                    // this snapshot

  int numServerCommands;     // text based server commands to execute when
                             // this
...
typedef enum cgameEvent_e {
  CGAME_EVENT_NONE = 0,
  CGAME_EVENT_GAMEVIEW = 1,
  CGAME_EVENT_SPEAKEREDITOR = 2,
  // unused, was CGAME_EVENT_CAMPAIGNBREIFING
...
typedef enum {
  ...
  CG_S_GETSOUNDLENGTH, // xkan - get the length (in milliseconds) of the
                       // sound
  ...
  CG_R_GETSKINMODEL,   // client allowed to view what the .skin loaded so
                       // they can set their model appropriately
  CG_R_GETMODELSHADER, // client allowed the shader handle for given
                       // model/surface (for things like debris
                       // inheriting shader from explosive)
...
typedef enum {
  ...
  // a console command has been issued locally that is not recognized by
  // the main game system. use Cmd_Argc() / Cmd_Argv() to read the
  // command, return qfalse if the command is not known to the game

  CG_DRAW_ACTIVE_FRAME,
  //	void (*CG_DrawActiveFrame)( int serverTime, stereoFrame_t
  // stereoView,
  // qboolean demoPlayback );
  // Generates and draws a game scene and status information at the
  // given time. If demoPlayback is set, local movement prediction will
  // not be enabled
