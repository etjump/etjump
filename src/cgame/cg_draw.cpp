  ...
  if (cgs.voteTime) {
    ...
    if (!(cg.snap->ps.eFlags & EF_VOTED)) {
      ...
      if (cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR &&
          etj_spectatorVote.integer <= 0) {
        line_b = ETJump::stringFormat("YES:%i, NO:%i (%s)", cgs.voteYes,
                                      cgs.voteNo, "Spectators can't vote");
      } else {
        line_b = ETJump::stringFormat("YES(%s):%i, NO(%s):%i", str1,
                                      cgs.voteYes, str2, cgs.voteNo);
      }
    } else {
      line_a =
      ...
      if (cgs.votedYes) {
        ...
        CG_DrawRect_FixedBorder(x_b + textWidth + 13, 214 - 10 + 12, 11, 12, 1,
                                color);
      }
    }
  }
...
static void drawProneIcon(float x, float y) {
  drawPic(x, y, ProneIconSize, ProneIconSize, cgs.media.proneIcon,
          ProneIconColor);
}
  ...
  if (ps->stats[STAT_USERCMD_BUTTONS] & WBUTTON_PRONE) {
    if (cg.time > cg.pronePressTime) {
      CG_CenterPrint("^7You cannot ^3prone ^7inside this area.",
                     SCREEN_HEIGHT - 88, SMALLCHAR_WIDTH);
