...
int CG_SoundScriptPrecache(const char *name) {
  ...
  for (sound = hashTable[hash]; sound; sound = sound->nextHash) {
    if (!Q_stricmp(s, sound->name)) {
      ...
      } else /*if (cg_buildScript.integer)*/ // Enabled
                                             // this
                                             // permanently
                                             // so that
                                             // streaming
...
==============
*/
int CG_SoundPickOldestRandomSound(soundScript_t *sound, vec3_t org,
                                  int entnum) {
...
int CG_SoundPlaySoundScript(const char *name, vec3_t org, int entnum,
  ...
  CG_Printf(S_COLOR_YELLOW "WARNING: CG_SoundPlaySoundScript: cannot "
                           "find sound script '%s'\n",
            name);
  return 0;
}
...
qboolean CG_SaveSpeakersToScript(void) {
  ...
  if (trap_FS_FOpenFile(va("sound/maps/%s.sps", cgs.rawmapname), &fh,
                        FS_WRITE) < 0) {
    CG_Printf(S_COLOR_RED "ERROR: failed to save speakers to "
                          "'sound/maps/%s.sps'\n",
              cgs.rawmapname);
    return qfalse;
  }
...
void CG_SpeakerEditorDraw(void) {
  ...
  if (!editSpeakerActive) {
    ...
    CG_Text_Paint_Ext(8, y, .2f, .2f, colour,
                      va("Current amount of speakers in map: %i (inpvs: %i "
                         "max in map: %i)",
...
void CG_UndoEditSpeaker(void) {
  ...
  if (undoSpeakerIndex == -1) {
    if (!BG_SS_StoreSpeaker(&undoSpeaker)) {
      CG_Printf(S_COLOR_YELLOW "UNDO: restoring deleted speaker failed, no "
                               "storage memory for speaker\n");
    } else {
      CG_Printf("UNDO: restored deleted speaker at "
                "%.2f %.2f %.2f.\n",
    ...
    CG_Printf("UNDO: restoring modified settings of speaker at "
              "%.2f %.2f %.2f.\n",
              undoSpeaker.origin[0], undoSpeaker.origin[1],
              undoSpeaker.origin[2]);
  }
...
static void CG_PlayScriptSpeaker(bg_speaker_t *speaker, qboolean global) {
  switch (speaker->loop) {
    ...
    case S_LT_LOOPED_OFF:
      ...
      ETJump::addRealLoopingSound(speaker->origin, vec3_origin, speaker->noise,
