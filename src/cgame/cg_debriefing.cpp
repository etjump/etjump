...
void CG_DebriefingPanel_Setup(std::vector<panel_button_t> &layout,
...
void CG_ChatPanel_Setup(void) {
  debriefPanelButtonsLayout.reserve(ETJump::nelem(debriefPanelButtons) *
  ...
  CG_DebriefingPanel_Setup(teamDebriefPanelButtonsLayout,
...
void CG_Debriefing_Shutdown(void) { cgs.dbShowing = qfalse; }
...
void CG_DebriefingTitle_Draw(panel_button_t *button) {
  ...
  if (cg_gameType.integer == GT_WOLF_STOPWATCH) {
    ...
    if (cgs.currentRound) {
      ...
      if (!defender) {
        if (winner != defender) {
          s = "ALLIES SUCCESSFULLY "
              "BEAT THE CLOCK!";
        } else {
          s = "ALLIES COULDN'T BEAT "
              "THE CLOCK!";
        }
      } else {
        if (winner != defender) {
          s = "AXIS SUCCESSFULLY BEAT "
              "THE CLOCK!";
        } else {
          s = "AXIS COULDN'T BEAT THE "
              "CLOCK!";
        }
      }
    }
  ...
  s = va("%s to next map",
         ETJump::getSecondsString(
             std::ceil(std::max(
                 60 - (cg.time - cgs.intermissionStartTime) / 1000.0, 0.0)))
             .c_str());
  w = CG_Text_Width_Ext(s, button->font->scalex, 0, button->font->font);
...
void CG_PanelButtonsRender_Window_Ext(rectDef_t *r, const char *text, int align,
                                      int innerheight, float fontscale,
                                      int yofs) {
...
void CG_Debreifing2_MissionTitle_Draw(panel_button_t *button) {
  ...
  s = va("%i SECS TO NEXT MAP",
         std::max(60 - (cg.time - cgs.intermissionStartTime) / 1000, 0));
        ...
        switch (CG_Debriefing_FindWinningTeamForPos(i + 1)) {
          case TEAM_AXIS:
            CG_DrawPic(x - 12, y - 12, 24, 24,
                       trap_R_RegisterShaderNoMip("gfx/"
                                                  "loading/"
                                                  "pin_"
                                                  "axis"));
            break;
  ...
  if (cg_gameType.integer == GT_WOLF_CAMPAIGN) {
    if (pos == 0) {
      ...
      /*			} else {
         ...
         CS_MULTI_MAPWINNER ); const char* buf =
         Info_ValueForKey( s, "winner" );
