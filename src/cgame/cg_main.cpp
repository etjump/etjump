...
cvarTable_t cvarTable[] =
{
	...
	{ &etj_disableOB, "etj_disableOB", "0", CVAR_ARCHIVE },
};
		...
		if (cv->vmCvar)
		{
			...
			if (cv->modificationCount != cv->vmCvar->modificationCount)
			{
				...
				if (cv->vmCvar == &cg_autoAction || cv->vmCvar == &cg_autoReload ||
					...
					cv->vmCvar == &etj_quickFollow || cv->vmCvar == &etj_disableOB
				    )
				{
					fSetFlags = qtrue;
				}
...
void CG_setClientFlags(void)
{
	...
	trap_Cvar_Set("cg_uinfo", va("%d %d %d %d %f %d",
	                             ...
	                             (
									 ...
									 ((etj_disableOB.integer > 0) ? CGF_DISABLE_OB : 0)
