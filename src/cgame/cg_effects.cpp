...
quads and not cap the start circle
    ...
    -- flags --
    SL_NOTRACE			- don't do a trace check for shortening the
beam, always draw at full 'range' length SL_TRACEWORLDONLY	- go through
everything
but the world SL_NODLIGHT			- don't put a dlight at the end
    SL_NOSTARTCAP		- dont' cap the start circle
    SL_LOCKTRACETORANGE	- only trace out as far as the specified range (rather
than to max spot range) SL_NOFLARE			- don't draw a flare
when the light is pointing at the camera SL_NOIMPACT			- don't
draw the impact mark on hit surfaces
    SL_LOCKUV			- lock the texture coordinates at the 'true'
length of the requested beam. SL_NOCORE			- don't draw the center
'core' beam
...
void CG_Spotlight(centity_t *cent, float *color, vec3_t realstart,
  ...
  vec3_t v1, v2;
  vec3_t startvec,
      endvec; // the vectors to rotate around lightDir to create the
              // circles
  vec3_t conevec;
  ...
  float hitDist; // the actual distance of the trace impact	(0 is no
                 // hit)
  ...
  {
    VectorMA(color, 1.0 / colorNorm[3], color,
             colorNorm); // FIXME: div by 0
  } else {
  ...
  if (flags & SL_LOCKTRACETORANGE) {
    VectorMA(start, range, lightDir,
             traceEnd); // trace out to 'range'
  } else {
  ...
  if (flags & SL_NOTRACE) {
    ...
    // MASK_ALL
    //&~(CONTENTS_MONSTERCLIP|CONTENTS_AREAPORTAL|CONTENTS_NOSAVE));
  }
  ...
  if (cent->currentState.modelindex) {
    ...
    // push start out a bit so the beam fits to the front of the
    // base model
  ...
  VectorMA(startvec, beamLen, conevec,
           endvec); // this applies the offset of the start diameter
                    // before finding the end points

  VectorScale(lightDir, beamLen, endCenter);
  VectorSubtract(endCenter, endvec,
                 coreverts[3].xyz); // get a vector of the radius out at
                                    // the end for the core to use
...
#define CORESCALE 0.6f
  ...
  if (!(flags & SL_NOIMPACT)) {
    if (hitDist) {
      ...
      //%	CG_ImpactMark(
      // cgs.media.spotLightShader, tr.endpos, proj, 0,
      // colorNorm[0], colorNorm[1], colorNorm[2],
      // alpha, qfalse, radius, qtrue,
      //-1 );
  ...
  if (!(flags & SL_NODLIGHT)) {
    vec3_t dlightLoc;
    VectorMA(tr.endpos, 0, lightDir,
             dlightLoc); // back away from the hit
  ...
  if (!(flags & SL_NOFLARE)) {
    ...
    // it's facing the camera, find out how closely and trace to
    // see if the source can be seen

    deg = RAD2DEG(M_PI - acos(dot));
    if (deg <= 35) // start flare a bit before the camera gets
                   // inside the cylinder
    {
      lightInEyes = qtrue;
      flarescale = 1 - (deg / 35);
    ...
    if (lightInEyes) {
      ...
      if (dist < 512) // make even bigger if you're
                      // close enough
      {
      ...
      // even though it's off, still need to add it, but
      // turned off so it can fade in/out properly
      trap_R_AddCoronaToScene(start, colorNorm[0], colorNorm[1], colorNorm[2],
