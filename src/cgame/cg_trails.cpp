...
trailJunc_t *CG_SpawnTrailJunc(trailJunc_t *headJunc) {
  ...
  j->nextJunc = headJunc; // if headJunc is NULL, then we'll just be the
                          // end of the list
...
#define ST_RATIO 4.0 // sprite image: width / height
  ...
  if (headJunc) {
    ...
    // first junction, so this will become the "tail" very soon,
    // make it fade out
    j->sTex = 0;
    j->alphaStart = 0.0;
    j->alphaEnd = 0.0;
...
#define TRAIL_FADE_FAR_SCALE 4.0
  ...
  if (!numJuncs) {
    ...
    while (j) {
      ...
      if (!j->inuse && j->nextJunc && !j->nextJunc->inuse) {
        ...
        // not sure how this can happen, but it
        // does, and causes infinite loops
        j->nextJunc = NULL;
      }
  ...
  if (trail->sType == STYPE_STRETCH) {
    // sInc = ((1.0 - 0.1) / (float)(numJuncs));	// hack, the
    // end of funnel shows a bit of the start (looping)
    s = 0.05;
    // s = 0.05;
  ...
  while (jNext) {
    ...
    // do fading when moving towards the projection point onto
    // the trail segment vector
  ...
  if (trail->flags & TJFL_FIXDISTORT) {
    // build the list of outVerts, by dividing up the QUAD's
    // into 4 Tri's each, so as to allow
    ...
    // send the polygons
    // FIXME: is it possible to send a GL_STRIP here? We are
    // actually sending 2x the verts we really need to
    if (!(trail->flags & TJFL_NOPOLYMERGE)) {
