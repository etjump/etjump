...
void CG_createStatsWindow(void) {
  cg_window_t *sw = CG_windowAlloc(WFX_TEXTSIZING | WFX_FADEIN |
                                       /*WFX_SCROLLUP|*/ WFX_TRUETYPE,
                                   110);
  ...
  //	sw->y = (cg.snap->ps.pm_type == PM_INTERMISSION) ? -20 : -7;
  //// Align
...
void CG_createTopShotsWindow(void) {
  cg_window_t *sw = CG_windowAlloc(WFX_TEXTSIZING | WFX_FLASH | WFX_FADEIN |
                                       WFX_SCROLLUP | WFX_TRUETYPE,
                                   190);
...
250); const helpType_t help[] =
    ...
    for(i=0; i<sizeof(help)/sizeof(helpType_t); i++) {
        if(help[i].cmd == NULL) {
            ...
            CG_printWindow(va(format, CG_getBindKeyName(help[i].cmd, buf,
sizeof(buf)), help[i].info));
        }
    }
}
*/
...
void CG_demoAviFPSDraw(void) {
  qboolean fKeyDown =
      (cgs.fKeyPressed[K_F1] | cgs.fKeyPressed[K_F2] | cgs.fKeyPressed[K_F3] |
       cgs.fKeyPressed[K_F4] | cgs.fKeyPressed[K_F5])
          ? qtrue
          : qfalse;
...
void CG_windowDraw(void) {
  ...
  // Mouse cursor position for MV highlighting (offset for cursor
  // pointer position) Also allow for swingcam toggling
  ...
  for (i = 0; i < cg.winHandler.numActiveWindows; i++) {
    ...
    if (w->state == WSTATE_START) {
      ...
      if (w->effects & WFX_SCROLLUP) {
        if (tmp > 0) {
          y = w->curY + (480 - w->y) * t_offset /
                            w->targetTime; //(100
                                           //* t_offset / w->targetTime) / 100;
        }
        if (tmp < 0 || y >= 480) {
          w->state = WSTATE_OFF;
          fCleanup = qtrue;
...
void CG_cursorUpdate(void) {
  ...
  for (i = wh->numActiveWindows - 1; i >= 0; i--) {
    ...
    if ((w->effects & WFX_MULTIVIEW) && w != cg.mvCurrentMainview) {
      ...
      // If the current window is selected, and the
      // button is down, then allow the update to occur,
      // as quick mouse movements can move it past the
      // window borders
      if (!fFound && (((w->mvInfo & MV_SELECTED) && fSelect) ||
        ...
        // If not dragging/resizing, prime for
        // later update
        if (!fSelect) {
          w->m_x = -1.0f;
          w->m_y = -1.0f;
        ...
        // Reset mouse info for window if it
        // loses focuse
  ...
  // Check MV overlay detection here for better perf with more text
  // elements General boundary check
  else {
