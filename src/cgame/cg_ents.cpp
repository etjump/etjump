...
//----(SA)	whoops, didn't mean to check it in with the missile flare
	...
	{
		team_t missileTeam = static_cast<team_t>(cent->currentState.weapon == WP_LANDMINE ? cent->currentState.teamNum % 4 : cent->currentState.teamNum);
...
//	CG_Printf("dot: %f\n", dot);
	...
	if (!behind && !toofar)
	{
		CG_Trace(&tr, cg.refdef_current->vieworg, NULL, NULL, cent->lerpOrigin, -1, MASK_SOLID);    // added blockage by players.  not sure how this is going to be since this is their bb, not their model (too much blockage)
