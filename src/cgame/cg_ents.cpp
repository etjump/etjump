...
// ydnar: dlight values go from 0-1.5ish
  ...
  // ydnar: if the dlight has angles, then it is a directional global
  // dlight
...
static void CG_EntityEffects(centity_t *cent) {
  ...
  if (cent->currentState.loopSound) {
    ...
    if (cent->currentState.eType == ET_SPEAKER) {
      ...
      if (cent->currentState.dmgFlags) // range is set
      {
        ...
        ETJump::addRealLoopingSound(
            cent->lerpOrigin, vec3_origin,
            cgs.gameSounds[cent->currentState.loopSound], 1250, volume,
            cent->soundTime);
      }
    } else if (cent->currentState.eType == ET_MOVER) {
      ETJump::addLoopingSound(cent->lerpOrigin, vec3_origin,
                              cgs.gameSounds[cent->currentState.loopSound],
      ...
      v = cgs.inlineModelMidpoints[cent->currentState.modelindex];
      VectorAdd(cent->lerpOrigin, v, origin);
      ETJump::addLoopingSound(origin, vec3_origin,
                              cgs.gameSounds[cent->currentState.loopSound],
                              cent->currentState.onFireStart, cent->soundTime);
    } else {
      ETJump::addLoopingSound(cent->lerpOrigin, vec3_origin,
  ...
  // DHM - Nerve :: If EF_SMOKING is set, emit smoke
    ...
    if (cent->lastTrailTime < cg.time) {
      ...
      CG_SmokePuff(cent->lerpOrigin, dir, 15 + (random() * 10), rnd, rnd, rnd,
                   0.4, 1500 + (rand() % 500), cg.time, cg.time + 500, 0,
                   cgs.media.smokePuffShader);
    }
  }
  // dhm - end

  // JPW NERVE same thing but for smoking barrels instead of nasty
  // server-side effect from single player
    ...
    if (cent->lastTrailTime < cg.time) {
      ...
      CG_SmokePuff(cent->lerpOrigin, dir,
                   40 + random() * 70, // 40+(rnd*40),
...
static void CG_General(centity_t *cent) {
  ...
  if (cent->currentState.eType == ET_MG42_BARREL) {
    ...
    if (cent->currentState.otherEntityNum == cg.snap->ps.clientNum &&
      ...
      // AnglesToAxis(
      // cg_entities[cent->currentState.otherEntityNum].lerpAngles,
      // ent.axis );
      AnglesToAxis(cent->lerpAngles, ent.axis);
    }
    /*		{
       ...
       ); CG_RailTrail2( NULL, cent->lerpOrigin, v ); return;
            }*/
  } else {
  ...
  if (cent->currentState.eType == ET_GAMEMODEL) {
    ...
    if (s1->torsoAnim) {
      if (cg.time >= cent->lerpFrame.frameTime) {
        ...
        while (cg.time >= cent->lerpFrame.frameTime &&
               // Mad Doc xkan, 11/18/2002 - if
               // teamNum == 1, then we are
               // supposed to stop the animation
               // when we reach the end of this
               // loop Gordon: 27/11/02:
               // clientNum already does this.
               // xkan, 1/8/2003 - No, it does
    ...
    // xkan, 11/27/2002 - only advance/change frame if the game
    // model has not been stopped (teamNum != 1)
  ...
  if (cent->currentState.powerups == STATE_UNDERCONSTRUCTION) {
    /*if( cent->currentState.solid == SOLID_BMODEL ) {
        ent.customShader =
    cgs.media.genericConstructionShaderBrush; } else {
        ent.customShader =
    cgs.media.genericConstructionShaderModel;
    }*/
...
qboolean CG_PlayerSeesItem(playerState_t *ps, entityState_t *item, int atTime,
  ...
  if (dist > 255) {
    return qfalse; // only run the remaining stuff on items that
                   // are close enough
  }
  ...
  // give more range based on distance (the hit area is wider when
  // closer)

  //	foo = -0.94f - (dist/255.0f) * 0.057f;	// (ranging from -0.94
  // to -0.997) (it happened to be a pretty good range)
  foo = -0.94f - (dist * (1.0f / 255.0f)) *
                     0.057f; // (ranging from -0.94 to -0.997) (it
                             // happened to be a pretty good range)
  ...
  // (SA) okay, everything else is okay, so do a bloody trace. (so
  // coronas on treasure doesn't show through walls) <sigh>
...
===============
*/
static void CG_DrawMineMarkerFlag(centity_t *cent, refEntity_t *ent,
                                  const weaponInfo_t *weapon) {
  entityState_t *s1;
...
static void CG_Missile(centity_t *cent) {
  ...
  //----(SA)	whoops, didn't mean to check it in with the missile
  // flare
  ...
  if (cent->currentState.eType == ET_FP_PARTS) {
    ...
    team_t missileTeam =
        static_cast<team_t>(cent->currentState.weapon == WP_LANDMINE
    ...
    if (cent->currentState.teamNum < 4) {
      ...
      if ((cgs.clientinfo[cg.snap->ps.clientNum].team !=
        ...
        if (cent->currentState.density - 1 == cg.snap->ps.clientNum) {
          ...
          // see if we have the skill to
          // see them and are close
          // enough
        ...
        ent.hilightIntensity = 0.5f *
        sin((cg.time-cent->highlightTime)/1000.f)
        + 1.f;*/
      }
    }
...
extern void CG_RunLerpFrame(centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf,
...
static void CG_TrapSetAnim(centity_t *cent, lerpFrame_t *lf, int newAnim) {
  ...
  // DHM - Nerve :: teamNum specifies which set of animations to use
  // (only 1 exists right now)
...
static void CG_Constructible(centity_t *cent) {
  ...
  if (s1->modelindex) {
    ...
    // ent.shaderRGBA[2] = 0xff; 	ent.shaderRGBA[3] = s1->density;

    // if( s1->angles2[0] < 255 )
    // if( cent->currentState.powerups ==
    // STATE_UNDERCONSTRUCTION )
    //	ent.customShader =
    // cgs.media.genericConstructionShaderBrush;
  ...
  if (s1->modelindex2) {
    if (cent->currentState.powerups == STATE_UNDERCONSTRUCTION) {
      ...
      /*switch( cent->currentState.frame ) {
          case 1: trap_S_AddLoopingSound(
      cent->currentState.origin2, vec3_origin,
      cgs.media.buildSound[0], 255, 0 ); break; case 2:
      trap_S_AddLoopingSound(
      cent->currentState.origin2, vec3_origin,
      cgs.media.buildSound[1], 255, 0 ); break; case 3:
      trap_S_AddLoopingSound(
      cent->currentState.origin2, vec3_origin,
      cgs.media.buildSound[2], 255, 0
      ); break; case 4: trap_S_AddLoopingSound(
      cent->currentState.origin2, vec3_origin,
      cgs.media.buildSound[3], 255, 0 ); break;
      }*/
    }
...
static void CG_Waypoint( centity_t *cent ) {
    ...
    switch( cent->currentState.frame ) {
        case WAYP_ATTACK: ent.customShader = cgs.media.waypointAttackShader;
break; case WAYP_DEFEND: ent.customShader = cgs.media.waypointDefendShader;
break; case WAYP_REGROUP: ent.customShader = cgs.media.waypointRegroupShader;
break;
...
static void CG_Mover(centity_t *cent) {
  ...
  /*	if(	cent->currentState.eType == ET_ALARMBOX) {
     ...
     VectorNormalize2(cent->lerpAngles, angNorm);
                  //		(origin, dir, speed, duration, count,
     'randscale') CG_AddBulletParticles( cent->lerpOrigin, angNorm, 2,
     800, 4, 16.0f ); trap_S_StartSound (NULL,
     cent->currentState.number, CHAN_AUTO, cgs.media.sparkSounds );
              }
          }
      }*/
}
...
void CG_Mover_PostProcess(centity_t *cent) {
  ...
  if (cg.snap->ps.eFlags & EF_MOUNTEDTANK &&
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
      // Gordon: is this entity mounted on a tank, and
      // attached to _OUR_ turret entity (which could be
      // us)
  ...
  // Gordon: if we (or someone we're spectating) is on this tank, recalc
  // our view values
  if (cg.snap->ps.eFlags & EF_MOUNTEDTANK) {
    centity_t *tank =
...
void CG_Beam_2(centity_t *cent) {
  ...
  //(int)origin[1], (int)origin[2], (int)origin2[0], (int)origin2[1],
  //(int)origin2[2] );
  AxisClear(ent.axis);
...
static void CG_InterpolateEntityPosition(centity_t *cent) {
  ...
  // it would be an internal error to find an entity that interpolates
  // without a snapshot ahead of the current one
  if (cg.nextSnap == NULL) {
    // DHM - Nerve :: FIXME? There are some cases when in Limbo
    // mode during a map restart
    //					that were tripping this
    // error.
    // CG_Error( "CG_InterpolateEntityPosition: cg.nextSnap ==
    // NULL" ); CG_Printf("CG_InterpolateEntityPosition:
    // cg.nextSnap == NULL");
    return;
  }
  ...
  // to not extrapolate player positions if more recent data is
  // available
...
void CG_AddCEntity(centity_t *cent) {
  ...
  // call the appropriate function which will add this entity to the
  // view accordingly
...
qboolean CG_AddLinkedEntity(centity_t *cent, qboolean ignoreframe, int atTime) {
  ...
  if (!ignoreframe) {
    ...
    // call the appropriate function which will add this entity
    // to the view accordingly
    CG_ProcessEntity(cent);
  }
...
qboolean CG_AddEntityToTag(centity_t *cent) {
  ...
  if (cent->currentState.eType != ET_PLAYER) {
    if (!cent->currentState.density) // this entity should rotate with it's
                                     // parent, but can turn around using it's
                                     // own angles
    { // Gordon: fixed to rotate about the object's axis, not
      // the world
      vec3_t mat[3], mat2[3];
...
void CG_AddPacketEntities(void) {
  ...
  // Gordon: changing to a single loop, child will request that their
  // parents are added first anyway
  for (num = 0; num < cg.snap->numEntities; num++) {
    CG_AddCEntity_Filter(&cg_entities[cg.snap->entities[num].number]);
  }

  // Gordon: Add tank bits as a second loop, to stop recursion problems
  // with tank bits not on base entity
  for (num = 0; num < cg.snap->numEntities; num++) {
...
void CG_AttachBitsToTank(centity_t *tank, refEntity_t *mg42base,
  ...
  if (tank->tankframe != cg.clientFrame) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
      ...
      // us)
      if ((cg_entities[i].currentValid ||
           (i == cg.snap->ps.clientNum && cg.renderingThirdPerson)) &&
          cg_entities[i].currentState.eFlags & EF_MOUNTEDTANK &&
