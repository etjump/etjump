...
typedef struct flameChunk_s {
  struct flameChunk_s *nextGlobal,
      *prevGlobal; // next junction in the global list it is in (free or
                   // used)
  struct flameChunk_s *nextFlameChunk; // next junction in the trail
  struct flameChunk_s *nextHead,
      *prevHead; // next head junc in the world

  qboolean inuse;
  qboolean dead; // set when a chunk is effectively inactive, but
                 // waiting to be freed
  ...
  float sizeRand;
  float sizeRate; // rate per ms, variable according to speed (larger if
                  // moving slower)
  vec3_t baseOrg;
  int baseOrgTime;
  vec3_t velDir;
  float velSpeed; // flame chunks should start with a fast velocity,
                  // then slow down if there is nothing behind them
                  // pushing them along
  float rollAngle;
...
typedef struct centFlameInfo_s {
  int lastClientFrame;          // client frame that we last fired the
                                // flamethrower
  ...
  int lastDmgUpdate; // time we last told server about this ent's flame
                     // damage
...
typedef struct {
  //	float fireVolume;	// not needed, since we add individual
  // loop
...
void CG_FireFlameChunks(centity_t *cent, vec3_t origin, vec3_t angles,
  ...
  // spawn the chunks that fired over the last frame
  if ((centInfo->lastClientFrame == cent->currentState.frame) &&
    ...
    while (t <= cg.time) {
      ...
      if (!f) {
        ...
        //  CHANGE: id
        //  to make sure we do not keep trying
      ...
      if (++numFrameChunks > 50) {
        // to make sure we do not keep trying to
        // add more and more chunks
        centInfo->lastFlameChunk->timeStart = cg.time;
        break;
      }
      // end CHANGE: id
...
void CG_MergeFlameChunks(flameChunk_t *f1, flameChunk_t *f2) {
  if (f1->nextFlameChunk != f2) {
    CG_Error("CG_MergeFlameChunks: f2 doesn't follow f1, "
             "cannot merge\n");
  }
...
void CG_MoveFlameChunk(flameChunk_t *f) {
  ...
  if (f->size < f->sizeMax) {
    if ((cg.time - f->timeStart) < f->blueLife) {
      f->sizeRate =
          GET_FLAME_BLUE_SIZE_SPEED(FLAME_START_MAX_SIZE); // use a constant so
                                                           // the blue flame
                                                           // doesn't distort
    } else {
...
void CG_AddFlameSpriteToScene(flameChunk_t *f, float lifeFrac, float alpha) {
  ...
  // find the projected distance from the eye to the projection of the
  // flame origin onto the view direction vector
  ...
  for (i = 0; i < numFrames; i++) {
    ...
    if (compressedVersionAvailable) {
      Com_sprintf(str, sizeof(str),
                  ...
                  d, dir, b, c, d, srcBlend, dstBlend, extras);
    } else {
      Com_sprintf(str, sizeof(str),
                  "%s%i\n{\n\tnofog%s\n\tallowCompress\n\tcull "
                  "none\n\t{\n\t\tmap "
    ...
    if (f->dead) {
      ...
      CG_FreeFlameChunk(f);
               ...
               (centFlameInfo[f->ownerCent].lastFlameChunk ==
                f)) // don't draw the ignition flame after we
                    // start firing
    {
...
void CG_UpdateFlamethrowerSounds(void) {
  ...
  while (f) {
    ...
    // traverse the chunks, spawning flame sound sources as we
    // go
