...
qboolean CG_FindCampaignInFile(char *filename, const char *campaignShortName,
  ...
  while (trap_PC_ReadToken(handle, &token)) {
    if (*token.string == '}') {
      ...
      if (!trap_PC_ReadToken(handle,
                             &token)) // don't do a stringparse due to
                                      // memory constraints
      {
      ...
      while (*ptr) {
        ...
        *mapnameptr = '\0';
        ...
        if (info->mapCount >= MAX_MAPS_PER_CAMPAIGN) {
          trap_Print(va(S_COLOR_RED "too many maps for a "
                                    "campaign inside: %s\n",
                        filename));
          trap_PC_FreeSource(handle);
          break;
...
void CG_LocateCampaign(void) {
  /*	int			numdirs;
      ...
      numdirs = trap_FS_GetFileList( "scripts", ".campaign", dirlist,
     1024 ); dirptr  = dirlist; for (i = 0; i < numdirs; i++, dirptr +=
     dirlen+1) { dirlen = strlen(dirptr); strcpy(filename, "scripts/");
     ...
     &cgs.campaignData.arenas[i] ) && !CG_FindArenaInfo(
     "scripts/wolfxp.arena", cgs.campaignData.mapnames[i],
     &cgs.campaignData.arenas[i] ) && !CG_FindArenaInfo( filename,
