...
TEST_F(TimeUtilitiesTests, Date_fromString_shouldParseDateCorrectly) {
  TimeUtils::Date expected{2021, 3, 5};
  ASSERT_EQ(TimeUtils::Date::fromString("2021-03-05"), expected);
}

TEST_F(TimeUtilitiesTests, Date_fromString_shouldThrowIfDateIsNotNumerical) {
  ASSERT_THROW(TimeUtils::Date::fromString("foobar"), std::invalid_argument);
}

TEST_F(TimeUtilitiesTests, Date_fromString_shouldThrowIfDayIsMissing) {
  ASSERT_THROW(TimeUtils::Date::fromString("2021-03"), std::invalid_argument);
}
